!function () {
    ...
    d3.behavior.zoom = function () {
        ...
        function rescale() {
            if (x1) x1.domain(x0.range().map(
                function (x) {
                    return (x - view.x) / view.k;
                }).map(x0.invert));
            if (y1) y1.domain(y0.range().map(
                function (y) {
    ...
    d3.dsv = function (delimiter, mimeType) {
        ...
        dsv.parse = function (text, f) {
            ...
            return dsv.parseRows(text, function (row, i) {
                ...
                var a = new Function("d", "return {" + row.map(
                    function (name, i) {
    ...
    d3.geo.graticule = function () {
        ...
        function lines() {
            return d3.range(Math.ceil(X0 / DX) * DX, X1, DX).map(X).concat(d3.range(Math.ceil(Y0 / DY) * DY, Y1, DY).map(Y)).concat(d3.range(Math.ceil(x0 / dx) * dx, x1, dx).filter(
                function (x) {
                    return abs(x % DX) > ε;
    ...
    d3.geom.voronoi = function (points) {
        ...
        voronoi.links = function (data) {
            return d3_geom_voronoi(sites(data)).edges.filter(
                function (edge) {
