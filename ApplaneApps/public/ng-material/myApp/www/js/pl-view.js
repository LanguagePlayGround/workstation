...
pl.controller('ChildViewCtrl', function ($scope, $timeout, $location) {
    var unwatcher = {};
            ...
            dModel.on("onSave", function (err, result, additionalResult) {
                if (err) {
                    if (err.validations) {
                        ...
                        var collection = fields[0].collectionid.collection;
...
//                throw new Error("No field found in getViewField >>>" + firstField + ">>>>viewFields>>>" + JSON.stringify(viewFields))
    ...
    $scope.getLookupData = function (viewValue, field, $fetchAllData, asFilter) {
        ...
        try {
            ...
            if (viewField.roleid && viewField.roleid.role) {
                resolvedParameters.__role__ = viewField.roleid.role;
            }
...
//        }
    ...
    function removeDollerFromStart(data, changeTo) {
    ...
    function populateCellTemplate(viewUI, field, selectedGroup, viewOptions) {
        ...
        if (field.referredWhen) {
            field.referredWhen = removeDollerFromStart(field.referredWhen);
        }
        ...
        if (field.colStyle) {
            field.colStyle = removeDollerFromStart(field.colStyle);
        }
        if (field.ui == "grid") {
            populateVisibililtyInNestedGrid(field.fields);
...
//                field.editableCellTemplate = "<pl-schedule data-model='row.entity' data-field='" + fieldAlias + "'></pl-schedule>";
            ...
            field.editableCellTemplate = "<div class='app-display-table-cell app-float-left app-width-74-per' >" +
                ...
                "                           <pl-autocomplete field='col' options='\"data as data for data in getLookupData($viewValue,col.field,$fetchAllData)\"' model='row.entity." + fieldAlias + ".unit'  ></pl-autocomplete>" +
                "                          </div>";
            ...
            field.editableCellTemplate = "<div class='app-display-table-cell app-float-left app-width-74-per' >" +
                ...
                "                         </div>" +
                "                        <div class='app-display-table-cell app-vertical-align-top pl-split-box' style='width:25%;border-right: 1px solid #ccc;'>" +
                "                           <pl-autocomplete field='col' options='\"data as data.currency for data in getLookupData($viewValue,col.field,$fetchAllData)\"' model='row.entity." + fieldAlias + ".type'  ></pl-autocomplete>" +
                "                       </div>";
            ...
            if (field.type == 'object') {
                field.html = removeDollerFromStart(field.html);
    ...
    function populateColumns(viewUI, fields, columns, aggregateColumns, groupColumns, filterColumns, sortColumns, updateColumns, ftsColumns, groups, tabs, parentField, viewOptions) {
        ...
        for (var i = 0; i < fieldLength; i++) {
            ...
            if (angular.isDefined(editable) && editable.toString().trim().length > 0) {
                ...
                field.editableWhen = removeDollerFromStart(field.editableWhen);
            } else {
            ...
            if (angular.isDefined(field.when) && (field.when).toString().trim().length > 0) {
                field.when = removeDollerFromStart(field.when);
            }
...
//        filterableColumn.reloadViewOnFilterChange = field.reloadViewOnFilterChange;
    ...
    function getHeaderActionFilter(filterColumns, actions) {                                                      // method add actions of filter type into filterColumns
        if (actions && actions.length > 0) {
            for (var i = 0; i < actions.length; i++) {
                if (actions[i].type == "filter") {
                    populateFilterColumns(actions[i], filterColumns);
                }
    ...
    function populatePageOptions() {
        if ($scope.view.viewOptions.sharedOptions.pageOptions && dataModel && dataModel.query) {
            ...
            if ($scope.view.viewOptions.fetchCount && dataModel.aggregateQuery && dataModel.aggregateData && dataModel.aggregateData.__count !== undefined) {
                $scope.view.viewOptions.sharedOptions.pageOptions.count = dataModel.aggregateData.__count;
    ...
    $scope.splitQviewHeader = function () {
        try {
            if ($scope.view.viewOptions && $scope.view.viewOptions.quickViewMenuGroup && $scope.view.viewOptions.quickViewMenuGroup.menus) {
                var toolbarWidth = $('.pl-header-toolbar').width();
...
//                $scope[$scope.view.viewOptions.openV](formView);
                $scope.workbenchOptions.currentView= formView;
                ...
                p = populateParametersFromQuery(p, currentRow);
                if (parameters) {
                    var newParameters = {};
                ...
                if (parameters && rowAction.currentRow) {
                    var newParameters = {};
                    var p = {};
                    p = populateParametersFromQuery(p, rowAction.currentRow);
    ...
    function populateParametersFromQuery(p, currentRow) {
        if (dataModel && dataModel.query && dataModel.query.$parameters) {
            p = angular.copy(dataModel.query.$parameters);
        }
        if (dataModel && dataModel.query && dataModel.query.$filter) {
            for (var key in dataModel.query.$filter) {
                if (p[key] === undefined) {
                    p[key] = dataModel.query.$filter[key];
                }
            }
        }
    ...
    $scope.populateUserPreferene = function (userPreferenceOptions, saveState) {
        try {
            if (!$scope.view.viewOptions.reloadViewOnFilterChange) {
                // change the parameters of the advance dashboard queries when filter changes.(manjeet-sangwan)
                if (dataModel instanceof DashboardDataModel) {
                ...
                } else if (dataModel instanceof DataModel && $scope.view.viewOptions.ui !== "dashboard") {
...
//                }
    ...
    function populateCrossTabData(params) {
        ...
        for (var i = 0; i < xaxisData.result.length; i++) {
            ...
            for (var j = 0; j < yaxisData.result.length; j++) {
                row.yaxis = row.yaxis || {};
                row.yaxis[j] = {};
                row.yaxis[j]["_id"] = yaxisData.result[j]._id;
                var found = false;
                for (var l = 0; l < data.length; l++) {
                    if ((Utility.deepEqual(xaxisData.result[i]._id, $scope.data[l][xAxisDataField]["_id"])) && ( Utility.deepEqual(yaxisData.result[j]._id, $scope.data[l][yAxisDataField]["_id"]))) {
                        for (var k = 0; k < dataFields.length; k++) {
                            ...
                            row.yaxis[j][dataField] = $scope.data[l][dataField];
                            row.yaxis[j]["data_id"] = $scope.data[l]._id;
                            found = true;
                        }
                        break;
                    }
                ...
                for (var n = 0; n < yAxisFields.length; n++) {
                    ...
                    row.yaxis[j][field] = yaxisData.result[j][field];
                }
                if (!found) {
                    row.yaxis[j]["data_id"] = Utility.getUniqueTempId();
                }
            }
    ...
    function populateCrossTabColumns(params) {
        ...
        for (var i = 0; i < yaxisData.result.length; i++) {
            var col = {field: i, type: "object", visibilityGrid: false};
    ...
    try {
        ...
        if (ui != "grid" && ui != "form" && ui != "dashboard" && ui != "html" && ui != "graph" && ui != "advance dashboard" && ui != 'aggregate') {
            var title = "ViewCtrl in pl.view";
        ...
        if (view.viewOptions.ui == 'grid') {
            ...
            if (viewOptions.actions) {
                for (var i = 0; i < viewOptions.actions.length; i++) {
                    ...
                    if (action.actionField) {
                        if (action.onRow) {
                            if (action.when) {
                                action.actionWhen = removeDollerFromStart(action.when);
                            }
                    ...
                    if (action.onRow || action.onHeader) {
                        if (action.onRow) {
                            ...
                            if (rowAction.when) {
                                rowAction.when = removeDollerFromStart(rowAction.when);
                            }
                        ...
                        if (action.onHeader && (action.visibility || action.visibilityGrid)) {
                            ...
                            if (headerAction.when) {
                                headerAction.when = removeDollerFromStart(headerAction.when, 'gridOptions.parameters.');
                            }
                        ...
                        if (action.when) {
                            action.when = removeDollerFromStart(action.when, 'gridOptions.parameters.');
                        }
                        if (action.when == undefined) {
                            action.when = true;
                        }
            ...
            populateColumns(ui, fields, columns, aggregateColumns, groupColumns, filterColumns, sortColumns, updateColumns, ftsColumns, undefined, tabs, undefined, viewOptions);
...
//            }
                    ...
                    if (angular.isDefined(newValue)) {
                        ...
                        if (currentRow) {
                            if ($parametermappings) {
                                var newParameters = {};
                                $scope.resolveParameters($parametermappings, currentRow, newParameters);
            ...
            viewOptions.qViewStyle = removeDollerFromStart(viewOptions.qViewStyle);
...
//                        }/* code may be required for Object case.*/
            ...
            if (viewOptions.actions) {
                for (var i = 0; i < viewOptions.actions.length; i++) {
                    ...
                    if (action.onRow && (action.visibility || action.visibilityForm)) {
                        if (action.when) {
                            action.when = removeDollerFromStart(action.when);
                        }
                        if (action.when == undefined) {
                            action.when = true;
                        }
                        ...
                        if (action.when) {
                            action.when = removeDollerFromStart(action.when, 'formOptions.parameters.');
                        }
                        if (action.when == undefined) {
                            action.when = true;
                        }
            ...
            for (var i = 0; i < groups.length; i++) {
                if (angular.isDefined(groups[i].when) && groups[i].when.toString().trim().length > 0) {
                    groups[i].when = removeDollerFromStart(groups[i].when);
                }
            ...
            var formOptions = {
                ...
                resizeV: viewOptions.resizeV,
                dataModel: dataModel,
                close: viewOptions.close !== undefined ? viewOptions.close : true,
            ...
            if (viewOptions.actions) {
                for (var i = 0; i < viewOptions.actions.length; i++) {
                    ...
                    if (action.type == "invoke") {
                        if (action.when) {
                            action.when = removeDollerFromStart(action.when, 'dashboardOptions.parameters.');
                        }
                        if (action.when == undefined) {
                            action.when = true;
                        }
            ...
            viewOptions.viewTemplate = "<div pl-html></div>";
        } else if (ui == "graph") {
            toolbarActions = [];
            if (viewOptions.actions) {
                for (var i = 0; i < viewOptions.actions.length; i++) {
...
pl.directive('plView', ["$compile", "$timeout", "$location", function ($compile, $timeout, $location) {
    return {
        restrict: "E",
...
//                    var template = ' <div class="bar bar-header bar-energized">'+
//                    '    <div class="buttons">'+
...
//                    '    </ion-refresher>'+
//                    $scope.view.viewOptions.viewTemplate+
//                    '    </ion-content>'+
//                    '    <ion-footer-bar align-title="left" class="bar bar-energized">'+
//                    '    <div class="buttons">'+
//                    '        <button class="button" ng-click="gridOptions.delete=!gridOptions.delete">Delete</button>'+
//                    '    </div>'+
//                    '    <h1 class="title"></h1>'+
//                    '    <div class="buttons" ng-click="doSomething()">'+
//                    '        <button class="button" ng-click="gridOptions.reorderRow=!gridOptions.reorderRow">Reorder</button>'+
//                    '    </div>'+
//                    '</ion-footer-bar>';
                    iElement.append($compile($scope.view.viewOptions.viewTemplate)($scope));
                    $timeout(function () {
