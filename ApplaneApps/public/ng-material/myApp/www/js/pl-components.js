        ...
        compile: function () {
            return {
                pre: function ($scope, iElement, attrs) {
                    ...
                    var ftsColumns = ftsInfo.columns;
                    var count = ftsColumns && ftsColumns.length > 0 ? ftsColumns.length : 0;
                    var ftsOptions = [];
                    for (var i = 0; i < ftsColumns.length; i++) {
                        var label = ftsColumns[i].label;
                        ftsOptions.push(label);
                    }
                    $scope.ftsField = {options: ftsOptions, fts: true};
...
//                            "           <span ng-repeat='otherCol in col.otherDisplayFields' ng-show='value[otherCol]'> | {{value[otherCol]}}</span>" +
                            ...
                            "       </div></div>" +
                            "       <input style='clear: both;' title='" + title + "' data-container='body' data-upsert='" + displayField + "' data-other-display-fields='" + otherdisplayfields + "' class='form-control' placeholder='" + placeholder + "' ng-model=" + model + " data-multiple='true'  data-animation='am-flip-x' ng-options='" + options + "' bs-typeahead type='text' />" +
                        ...
                        template = "<input title='" + title + "'  data-container='body' data-other-display-fields='" + otherdisplayfields + "' placeholder='" + placeholder + "' class='form-control' data-upsert='" + displayField + "' ng-model=" + model + "  data-animation='am-flip-x' ng-options='" + options + "' bs-typeahead type='text' />" +
                            "<input class='second-input' ng-class='{\"pl-full-right\":!" + field.upsertView + "}' type='text' tabindex='-1' style='width:20px;' >" +
                            "<img src='../images/loading.gif' class='pl-loading-image' ng-show='loadingImage' />" +
...
//                            "<input data-animation='am-flip-x' title='" + title + "' data-container='body' placeholder='" + placeholder + "'  type='text' ng-model='" + nextRunOnExpression + "' bs-datepicker  data-date-format='dd/MM/yyyy' />" +
                        ...
                        template += "<input data-animation='am-flip-x' title='" + title + "' data-container='body' placeholder='" + placeholder + "'  type='text' ng-model='" + modelExpression + "' bs-datepicker  data-date-format='dd/MM/yyyy' />" +
    ...
    function ($timeout) {
        ...
        return {
            ...
            link: function ($scope, el, attrs) {
                ...
                /* Style setup */
                sliderWrapper[0].style.transitionDuration = param.speed + 's';
                sliderWrapper[0].style.webkitTransitionDuration = param.speed + 's';
                sliderWrapper[0].style.zIndex = 1000;
                sliderWrapper[0].style.position = 'fixed';
                sliderWrapper[0].style.transitionProperty = 'width, height';
                ...
                /*                slider.style.transitionDuration = param.speed + 's';
                 slider.style.transitionProperty = 'height, left';
                 slider.style.webkitTransitionDuration = param.speed + 's';    // removed due issuse found with XP chrome on Lokesh gurgoan's system 02/02/2015// */
