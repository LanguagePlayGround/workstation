...
pl.controller('WorkbenchCtrl', function ($scope, $compile, $timeout, $parse, $q, $location, $http) {
    ApplaneDB.$http = $http;
    $scope.leftZero = "0px";
    $scope.rightZero = "0px";
    $scope.leftChild = "65%";
        ...
        if (angular.isDefined(err)) {
            ...
            if (err && err.clientError) {
                ...
                userDb.invokeFunction("Porting.insertClientErrorInServiceLogs", [
                    {"error": {"msg": err.message, "stack": err.stack, "selectedapplication": selectedViewInfo.application, "selectedmenu": selectedViewInfo.menu, selectedqview: selectedViewInfo.qview, "viewIds": selectedViewInfo.viewId}, "errorType": "Client", errorMessage: err.message}
                ]);
            }
        } else {
...
//        }
    ...
    $scope.openV = function (v, callback) {
        if ((!v.viewOptions) && AppViews[v.id]) {
        ...
        return  ensureRoadMapDB($scope.getRequestParameters()).then(
            ...
            function (userInfo) {
                ...
                $scope.workbenchOptions.userMenuGroup.menus = [
                    ...
                    {label: "Roadmap", onClick: "openChildWindow", roadmap: true},
...
//                    $scope.workbenchOptions.userMenuGroup.menus.splice(1, 0, {label: userInfo.orgName});
    ...
    $scope.getProcesses = function () {
        ...
        return userDb.invokeFunction("Porting.getProcessInfo", [$scope.workbenchOptions.pendingProcess]).then(function (processInfo) {
