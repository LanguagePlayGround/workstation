...
//                var item = Array.prototype.pop.call(remove);    // Array.prototype.pop.call(remove) was not working in Chrome---added
                var item = remove[i];
                if (item !== undefined) {
                    if (item.parentNode !== null && item.parentNode !== undefined) { // check that the item was actually added to DOM
...
//                            returnMethods.send(options.adapter);
                ...
                close: function () {
                    ...
                    //highlighted & blackout portion was not going so close function is called in recursion until these portion is not removed---Added
                    var documentHighlightedChild = document.getElementsByClassName("feedback-highlighted");
                    var documentBlackoutChild = document.getElementsByClassName("feedback-blackedout");
                    if ((documentHighlightedChild && documentHighlightedChild.length > 0) || (documentBlackoutChild && documentBlackoutChild.length > 0)) {
                        returnMethods.close();
                    } else {
                        return false;
                    }

//                    var sheet=document.getElementById("feedbackCss");
...
//                    return false;
                ...
                send: function () {
                    ...
                    adapter.send(data, errorStack, function (success, errMsg) {
                        ...
                        if (success === true) {
                            ...
                            var msgToShow = options.messageError;
                            if (errMsg) {
                                msgToShow = msgToShow + "(" + errMsg + ")";
                            }
                            modalBody.appendChild(document.createTextNode(msgToShow));
                        }
        ...
        //For not showing the Send Feedback button on bottom right-----added
//        if (options.appendTo !== null) {
//            ((options.appendTo !== undefined) ? options.appendTo : document.body).appendChild(button);
//        }

        //to open the popup window from the Applane Framework
        returnMethods.open();
        ...
        for (; i < len; i++) {
            ...
            switch (item.type) {
                case "text":
                    ...
                    inputElement.style = "margin-left:5px"
                    this.dom.appendChild(( item.element = inputElement));
                    break;
...
//                dom.appendChild(document.createElement("hr"));
    ...
    window.Feedback.Screenshot.prototype.start = function (modal, modalHeader, modalFooter, nextButton) {
        ...
        this._data = undefined;    //This line is added because dom was taking old data----added
        if (this.h2cDone) {
            emptyElements(this.dom);
            nextButton.disabled = false;
    ...
    window.Feedback.Screenshot.prototype.data = function () {
        ...
        if (this._data !== undefined) {
//            alert("retung data from scren shot")
            return this._data;
        }
    ...
    window.Feedback.XHR.prototype.send = function (data, errorStack, callback) {

//        console.log("data ................" + JSON.stringify(data));

        // This was added so that we can save the image through--framework----------added
        var subject = data[0].Subject;
        var issue = data[0].Issue;
        var fileName = "issue.png";
        var contents = data[1];
        var token = undefined;
        ...
        return ApplaneDB.getRoadMapConnection().then(
            function (result) {
                ...
                return ApplaneDB.connection("userdb").uploadFile(fileName, "image/png", contents, {token: token})
            }).then(function (result) {
                var insert = { task: subject, details: issue, file: {key: result.response[0].key, name: fileName}};
                if (errorStack != undefined) {
                    insert.errorMessage = errorStack;
                }
                insert.isRoadMap = true;
                var update = [
                    {$collection: "new_tasks", $insert: insert}
                ];

                return ApplaneDB.connection("userdb").update(update, {token: token})
