...
return bbox}
return tip};}));(function(root,factory){if(typeof define==='function'&&define.amd){define([],factory);}else if(typeof exports==='object'){module.exports=factory(require());}else{root.d3pie=factory(root);}}(this,function(){var _scriptName="d3pie";var _version="0.1.5";var _uniqueIDCounter=0;var defaultSettings={header:{title:{text:"",color:"#333333",fontSize:18,font:"arial"},subtitle:{text:"",color:"#666666",fontSize:14,font:"arial"},location:"top-center",titleSubtitlePadding:8},footer:{text:"",color:"#666666",fontSize:14,font:"arial",location:"left"},size:{canvasHeight:500,canvasWidth:500,pieInnerRadius:"0%",pieOuterRadius:null},data:{sortOrder:"none",ignoreSmallSegments:{enabled:false,valueType:"percentage",value:null},smallSegmentGrouping:{enabled:false,value:1,valueType:"percentage",label:"Other",color:"#cccccc"},content:[]},labels:{outer:{format:"label",hideWhenLessThanPercentage:null,pieDistance:30},inner:{format:"percentage",hideWhenLessThanPercentage:null},mainLabel:{color:"#333333",font:"arial",fontSize:10},percentage:{color:"#dddddd",font:"arial",fontSize:10,decimalPlaces:0},value:{color:"#cccc44",font:"arial",fontSize:10},lines:{enabled:true,style:"curved",color:"segment"},truncation:{enabled:false,length:30}},effects:{load:{effect:"default",speed:1000},pullOutSegmentOnClick:{effect:"bounce",speed:300,size:10},highlightSegmentOnMouseover:true,highlightLuminosity:-0.2},tooltips:{enabled:false,type:"placeholder",string:"",placeholderParser:null,styles:{fadeInSpeed:250,backgroundColor:"#000000",backgroundOpacity:0.5,color:"#efefef",borderRadius:2,font:"arial",fontSize:10,padding:4}},misc:{colors:{background:null,segments:["#2484c1","#65a620","#7b6888","#a05d56","#961a1a","#d8d23a","#e98125","#d0743c","#635222","#6ada6a","#0c6197","#7d9058","#207f33","#44b9b0","#bca44a","#e4a14b","#a3acb2","#8cc3e9","#69a6f9","#5b388f","#546e91","#8bde95","#d2ab58","#273c71","#98bf6e","#4daa4b","#98abc5","#cc1010","#31383b","#006391","#c2643f","#b0a474","#a5a39c","#a9c2bc","#22af8c","#7fcecf","#987ac6","#3d3b87","#b77b1c","#c9c2b6","#807ece","#8db27c","#be66a2","#9ed3c6","#00644b","#005064","#77979f","#77e079","#9c73ab","#1f79a7"],segmentStroke:"#ffffff"},gradient:{enabled:false,percentage:95,color:"#000000"},canvasPadding:{top:5,right:5,bottom:5,left:5},pieCenterOffset:{x:0,y:0},cssPrefix:null},callbacks:{onload:null,onMouseoverSegment:null,onMouseoutSegment:null,onClickSegment:null}};var validate={initialCheck:function(pie){var cssPrefix=pie.cssPrefix;var element=pie.element;var options=pie.options;if(!window.d3||!window.d3.hasOwnProperty("version")){console.error("d3pie error: d3 is not available");return false;}
if(!(element instanceof HTMLElement)){console.error("d3pie error: the first d3pie() param must be a valid DOM element (not jQuery) or a ID string.");return false;}
if(!(/[a-zA-Z][a-zA-Z0-9_-]*$/.test(cssPrefix))){console.error("d3pie error: invalid options.misc.cssPrefix");return false;}
if(!helpers.isArray(options.data.content)){console.error("d3pie error: invalid config structure: missing data.content property.");return false;}
if(options.data.content.length===0){console.error("d3pie error: no data supplied.");return false;}
var data=[];for(var i=0;i<options.data.content.length;i++){if(typeof options.data.content[i].value!=="number"||isNaN(options.data.content[i].value)){console.log("not valid: ",options.data.content[i]);continue;}
if(options.data.content[i].value<=0){console.log("not valid - should have positive value: ",options.data.content[i]);continue;}
data.push(options.data.content[i]);}
pie.options.data.content=data;return true;}};var helpers={addSVGSpace:function(pie){var element=pie.element;var canvasWidth=pie.options.size.canvasWidth;var canvasHeight=pie.options.size.canvasHeight;var backgroundColor=pie.options.misc.colors.background;var svg=d3.select(element).append("svg:svg").attr("viewBox","0 0 "+canvasWidth+" "+canvasHeight).attr("preserveAspectRatio","xMinYMin meet").append("g").attr("transform","translate(0,0)");if(backgroundColor!=="transparent"){svg.style("background-color",function(){return backgroundColor;});}
return svg;},whenIdExists:function(id,callback){var inc=1;var giveupIterationCount=1000;var interval=setInterval(function(){if(document.getElementById(id)){clearInterval(interval);callback();}
if(inc>giveupIterationCount){clearInterval(interval);}
inc++;},1);},whenElementsExist:function(els,callback){var inc=1;var giveupIterationCount=1000;var interval=setInterval(function(){var allExist=true;for(var i=0;i<els.length;i++){if(!document.getElementById(els[i])){allExist=false;break;}}
if(allExist){clearInterval(interval);callback();}
if(inc>giveupIterationCount){clearInterval(interval);}
inc++;},1);},shuffleArray:function(array){var currentIndex=array.length,tmpVal,randomIndex;while(0!==currentIndex){randomIndex=Math.floor(Math.random()*currentIndex);currentIndex-=1;tmpVal=array[currentIndex];array[currentIndex]=array[randomIndex];array[randomIndex]=tmpVal;}
return array;},processObj:function(obj,is,value){if(typeof is==='string'){return helpers.processObj(obj,is.split('.'),value);}else if(is.length===1&&value!==undefined){obj[is[0]]=value;return obj[is[0]];}else if(is.length===0){return obj;}else{return helpers.processObj(obj[is[0]],is.slice(1),value);}},getDimensions:function(id){var el=document.getElementById(id);var w=0,h=0;if(el){var dimensions=el.getBBox();w=dimensions.width;h=dimensions.height;}else{console.log("error: getDimensions() "+id+" not found.");}
return{w:w,h:h};},rectIntersect:function(r1,r2){var returnVal=((r2.x>(r1.x+r1.w))||((r2.x+r2.w)<r1.x)||((r2.y+r2.h)<r1.y)||(r2.y>(r1.y+r1.h)));return!returnVal;},getColorShade:function(hex,lum){hex=String(hex).replace(/[^0-9a-f]/gi,'');if(hex.length<6){hex=hex[0]+hex[0]+hex[1]+hex[1]+hex[2]+hex[2];}
lum=lum||0;var newHex="#";for(var i=0;i<3;i++){var c=parseInt(hex.substr(i*2,2),16);c=Math.round(Math.min(Math.max(0,c+(c*lum)),255)).toString(16);newHex+=("00"+c).substr(c.length);}
return newHex;},initSegmentColors:function(pie){var data=pie.options.data.content;var colors=pie.options.misc.colors.segments;var finalColors=[];for(var i=0;i<data.length;i++){if(data[i].hasOwnProperty("color")){finalColors.push(data[i].color);}else{finalColors.push(colors[i]);}}
return finalColors;},applySmallSegmentGrouping:function(data,smallSegmentGrouping){var totalSize;if(smallSegmentGrouping.valueType==="percentage"){totalSize=math.getTotalPieSize(data);}
var newData=[];var groupedData=[];var totalGroupedData=0;for(var i=0;i<data.length;i++){if(smallSegmentGrouping.valueType==="percentage"){var dataPercent=(data[i].value/totalSize)*100;if(dataPercent<=smallSegmentGrouping.value){groupedData.push(data[i]);totalGroupedData+=data[i].value;continue;}
data[i].isGrouped=false;newData.push(data[i]);}else{if(data[i].value<=smallSegmentGrouping.value){groupedData.push(data[i]);totalGroupedData+=data[i].value;continue;}
data[i].isGrouped=false;newData.push(data[i]);}}
if(groupedData.length){newData.push({color:smallSegmentGrouping.color,label:smallSegmentGrouping.label,value:totalGroupedData,isGrouped:true,groupedData:groupedData});}
return newData;},showPoint:function(svg,x,y){svg.append("circle").attr("cx",x).attr("cy",y).attr("r",2).style("fill","black");},isFunction:function(functionToCheck){var getType={};return functionToCheck&&getType.toString.call(functionToCheck)==='[object Function]';},isArray:function(o){return Object.prototype.toString.call(o)==='[object Array]';}};var extend=function(){var options,name,src,copy,copyIsArray,clone,target=arguments[0]||{},i=1,length=arguments.length,deep=false,toString=Object.prototype.toString,hasOwn=Object.prototype.hasOwnProperty,class2type={"[object Boolean]":"boolean","[object Number]":"number","[object String]":"string","[object Function]":"function","[object Array]":"array","[object Date]":"date","[object RegExp]":"regexp","[object Object]":"object"},jQuery={isFunction:function(obj){return jQuery.type(obj)==="function";},isArray:Array.isArray||function(obj){return jQuery.type(obj)==="array";},isWindow:function(obj){return obj!==null&&obj===obj.window;},isNumeric:function(obj){return!isNaN(parseFloat(obj))&&isFinite(obj);},type:function(obj){return obj===null?String(obj):class2type[toString.call(obj)]||"object";},isPlainObject:function(obj){if(!obj||jQuery.type(obj)!=="object"||obj.nodeType){return false;}
try{if(obj.constructor&&!hasOwn.call(obj,"constructor")&&!hasOwn.call(obj.constructor.prototype,"isPrototypeOf")){return false;}}catch(e){return false;}
var key;for(key in obj){}
return key===undefined||hasOwn.call(obj,key);}};if(typeof target==="boolean"){deep=target;target=arguments[1]||{};i=2;}
if(typeof target!=="object"&&!jQuery.isFunction(target)){target={};}
if(length===i){target=this;--i;}
for(i;i<length;i++){if((options=arguments[i])!==null){for(name in options){src=target[name];copy=options[name];if(target===copy){continue;}
if(deep&&copy&&(jQuery.isPlainObject(copy)||(copyIsArray=jQuery.isArray(copy)))){if(copyIsArray){copyIsArray=false;clone=src&&jQuery.isArray(src)?src:[];}else{clone=src&&jQuery.isPlainObject(src)?src:{};}
target[name]=extend(deep,clone,copy);}else if(copy!==undefined){target[name]=copy;}}}}
return target;};var math={toRadians:function(degrees){return degrees*(Math.PI/180);},toDegrees:function(radians){return radians*(180/Math.PI);},computePieRadius:function(pie){var size=pie.options.size;var canvasPadding=pie.options.misc.canvasPadding;var w=size.canvasWidth-canvasPadding.left-canvasPadding.right;var h=size.canvasHeight-canvasPadding.top-canvasPadding.bottom;if(pie.options.header.location!=="pie-center"){h-=pie.textComponents.headerHeight;}
if(pie.textComponents.footer.exists){h-=pie.textComponents.footer.h;}
h=(h<0)?0:h;var outerRadius=((w<h)?w:h)/3;var innerRadius,percent;if(size.pieOuterRadius!==null){if(/%/.test(size.pieOuterRadius)){percent=parseInt(size.pieOuterRadius.replace(/[\D]/,""),10);percent=(percent>99)?99:percent;percent=(percent<0)?0:percent;var smallestDimension=(w<h)?w:h;if(pie.options.labels.outer.format!=="none"){var pieDistanceSpace=parseInt(pie.options.labels.outer.pieDistance,10)*2;if(smallestDimension-pieDistanceSpace>0){smallestDimension-=pieDistanceSpace;}}
outerRadius=Math.floor((smallestDimension/100)*percent)/2;}else{outerRadius=parseInt(size.pieOuterRadius,10);}}
if(/%/.test(size.pieInnerRadius)){percent=parseInt(size.pieInnerRadius.replace(/[\D]/,""),10);percent=(percent>99)?99:percent;percent=(percent<0)?0:percent;innerRadius=Math.floor((outerRadius/100)*percent);}else{innerRadius=parseInt(size.pieInnerRadius,10);}
pie.innerRadius=innerRadius;pie.outerRadius=outerRadius;},getTotalPieSize:function(data){var totalSize=0;for(var i=0;i<data.length;i++){totalSize+=data[i].value;}
return totalSize;},sortPieData:function(pie){var data=pie.options.data.content;var sortOrder=pie.options.data.sortOrder;switch(sortOrder){case"none":break;case"random":data=helpers.shuffleArray(data);break;case"value-asc":data.sort(function(a,b){return(a.value<b.value)?-1:1;});break;case"value-desc":data.sort(function(a,b){return(a.value<b.value)?1:-1;});break;case"label-asc":data.sort(function(a,b){return(a.label.toLowerCase()>b.label.toLowerCase())?1:-1;});break;case"label-desc":data.sort(function(a,b){return(a.label.toLowerCase()<b.label.toLowerCase())?1:-1;});break;}
return data;},getPieTranslateCenter:function(pieCenter){return"translate("+pieCenter.x+","+pieCenter.y+")";},calculatePieCenter:function(pie){var pieCenterOffset=pie.options.misc.pieCenterOffset;var hasTopTitle=(pie.textComponents.title.exists&&pie.options.header.location!=="pie-center");var hasTopSubtitle=(pie.textComponents.subtitle.exists&&pie.options.header.location!=="pie-center");var headerOffset=pie.options.misc.canvasPadding.top;if(hasTopTitle&&hasTopSubtitle){headerOffset+=pie.textComponents.title.h+pie.options.header.titleSubtitlePadding+pie.textComponents.subtitle.h;}else if(hasTopTitle){headerOffset+=pie.textComponents.title.h;}else if(hasTopSubtitle){headerOffset+=pie.textComponents.subtitle.h;}
var footerOffset=0;if(pie.textComponents.footer.exists){footerOffset=pie.textComponents.footer.h+pie.options.misc.canvasPadding.bottom;}
var x=((pie.options.size.canvasWidth-pie.options.misc.canvasPadding.left-pie.options.misc.canvasPadding.right)/2)+pie.options.misc.canvasPadding.left;var y=((pie.options.size.canvasHeight-footerOffset-headerOffset)/2)+headerOffset;x+=pieCenterOffset.x;y+=pieCenterOffset.y;pie.pieCenter={x:x,y:y};},rotate:function(x,y,xm,ym,a){a=a*Math.PI/180;var cos=Math.cos,sin=Math.sin,xr=(x-xm)*cos(a)-(y-ym)*sin(a)+xm,yr=(x-xm)*sin(a)+(y-ym)*cos(a)+ym;return{x:xr,y:yr};},translate:function(x,y,d,a){var rads=math.toRadians(a);return{x:x+d*Math.sin(rads),y:y-d*Math.cos(rads)};},pointIsInArc:function(pt,ptData,d3Arc){var r1=d3Arc.innerRadius()(ptData),r2=d3Arc.outerRadius()(ptData),theta1=d3Arc.startAngle()(ptData),theta2=d3Arc.endAngle()(ptData);var dist=pt.x*pt.x+pt.y*pt.y,angle=Math.atan2(pt.x,-pt.y);angle=(angle<0)?(angle+Math.PI*2):angle;return(r1*r1<=dist)&&(dist<=r2*r2)&&(theta1<=angle)&&(angle<=theta2);}};var labels={add:function(pie,section,sectionDisplayType){var include=labels.getIncludes(sectionDisplayType);var settings=pie.options.labels;var outerLabel=pie.svg.insert("g","."+pie.cssPrefix+"labels-"+section).attr("class",pie.cssPrefix+"labels-"+section);var labelGroup=outerLabel.selectAll("."+pie.cssPrefix+"labelGroup-"+section).data(pie.options.data.content).enter().append("g").attr("id",function(d,i){return pie.cssPrefix+"labelGroup"+i+"-"+section;}).attr("data-index",function(d,i){return i;}).attr("class",pie.cssPrefix+"labelGroup-"+section).style("opacity",0);if(include.mainLabel){labelGroup.append("text").attr("id",function(d,i){return pie.cssPrefix+"segmentMainLabel"+i+"-"+section;}).attr("class",pie.cssPrefix+"segmentMainLabel-"+section).text(function(d){var str=d.label;if(settings.truncation.enabled&&d.label.length>settings.truncation.length){str=d.label.substring(0,settings.truncation.length)+"...";}
return str;}).style("font-size",settings.mainLabel.fontSize+"px").style("font-family",settings.mainLabel.font).style("fill",settings.mainLabel.color);}
if(include.percentage){labelGroup.append("text").attr("id",function(d,i){return pie.cssPrefix+"segmentPercentage"+i+"-"+section;}).attr("class",pie.cssPrefix+"segmentPercentage-"+section).text(function(d,i){return segments.getPercentage(pie,i)+"%";}).style("font-size",settings.percentage.fontSize+"px").style("font-family",settings.percentage.font).style("fill",settings.percentage.color);}
if(include.value){labelGroup.append("text").attr("id",function(d,i){return pie.cssPrefix+"segmentValue"+i+"-"+section;}).attr("class",pie.cssPrefix+"segmentValue-"+section).text(function(d){return d.value;}).style("font-size",settings.value.fontSize+"px").style("font-family",settings.value.font).style("fill",settings.value.color);}},positionLabelElements:function(pie,section,sectionDisplayType){labels["dimensions-"+section]=[];var labelGroups=d3.selectAll("."+pie.cssPrefix+"labelGroup-"+section);labelGroups.each(function(d,i){var mainLabel=d3.select(this).selectAll("."+pie.cssPrefix+"segmentMainLabel-"+section);var percentage=d3.select(this).selectAll("."+pie.cssPrefix+"segmentPercentage-"+section);var value=d3.select(this).selectAll("."+pie.cssPrefix+"segmentValue-"+section);labels["dimensions-"+section].push({mainLabel:(mainLabel.node()!==null)?mainLabel.node().getBBox():null,percentage:(percentage.node()!==null)?percentage.node().getBBox():null,value:(value.node()!==null)?value.node().getBBox():null});});var singleLinePad=5;var dims=labels["dimensions-"+section];switch(sectionDisplayType){case"label-value1":d3.selectAll("."+pie.cssPrefix+"segmentValue-"+section).attr("dx",function(d,i){return dims[i].mainLabel.width+singleLinePad;});break;case"label-value2":d3.selectAll("."+pie.cssPrefix+"segmentValue-"+section).attr("dy",function(d,i){return dims[i].mainLabel.height;});break;case"label-percentage1":d3.selectAll("."+pie.cssPrefix+"segmentPercentage-"+section).attr("dx",function(d,i){return dims[i].mainLabel.width+singleLinePad;});break;case"label-percentage2":d3.selectAll("."+pie.cssPrefix+"segmentPercentage-"+section).attr("dx",function(d,i){return(dims[i].mainLabel.width/2)-(dims[i].percentage.width/2);}).attr("dy",function(d,i){return dims[i].mainLabel.height;});break;}},computeLabelLinePositions:function(pie){pie.lineCoordGroups=[];d3.selectAll("."+pie.cssPrefix+"labelGroup-outer").each(function(d,i){return labels.computeLinePosition(pie,i);});},computeLinePosition:function(pie,i){var angle=segments.getSegmentAngle(i,pie.options.data.content,pie.totalSize,{midpoint:true});var originCoords=math.rotate(pie.pieCenter.x,pie.pieCenter.y-pie.outerRadius,pie.pieCenter.x,pie.pieCenter.y,angle);var heightOffset=pie.outerLabelGroupData[i].h/5;var labelXMargin=6;var quarter=Math.floor(angle/90);var midPoint=4;var x2,y2,x3,y3;if(quarter===2&&angle===180){quarter=1;}
switch(quarter){case 0:x2=pie.outerLabelGroupData[i].x-labelXMargin-((pie.outerLabelGroupData[i].x-labelXMargin-originCoords.x)/2);y2=pie.outerLabelGroupData[i].y+((originCoords.y-pie.outerLabelGroupData[i].y)/midPoint);x3=pie.outerLabelGroupData[i].x-labelXMargin;y3=pie.outerLabelGroupData[i].y-heightOffset;break;case 1:x2=originCoords.x+(pie.outerLabelGroupData[i].x-originCoords.x)/midPoint;y2=originCoords.y+(pie.outerLabelGroupData[i].y-originCoords.y)/midPoint;x3=pie.outerLabelGroupData[i].x-labelXMargin;y3=pie.outerLabelGroupData[i].y-heightOffset;break;case 2:var startOfLabelX=pie.outerLabelGroupData[i].x+pie.outerLabelGroupData[i].w+labelXMargin;x2=originCoords.x-(originCoords.x-startOfLabelX)/midPoint;y2=originCoords.y+(pie.outerLabelGroupData[i].y-originCoords.y)/midPoint;x3=pie.outerLabelGroupData[i].x+pie.outerLabelGroupData[i].w+labelXMargin;y3=pie.outerLabelGroupData[i].y-heightOffset;break;case 3:var startOfLabel=pie.outerLabelGroupData[i].x+pie.outerLabelGroupData[i].w+labelXMargin;x2=startOfLabel+((originCoords.x-startOfLabel)/midPoint);y2=pie.outerLabelGroupData[i].y+(originCoords.y-pie.outerLabelGroupData[i].y)/midPoint;x3=pie.outerLabelGroupData[i].x+pie.outerLabelGroupData[i].w+labelXMargin;y3=pie.outerLabelGroupData[i].y-heightOffset;break;}
if(pie.options.labels.lines.style==="straight"){pie.lineCoordGroups[i]=[{x:originCoords.x,y:originCoords.y},{x:x3,y:y3}];}else{pie.lineCoordGroups[i]=[{x:originCoords.x,y:originCoords.y},{x:x2,y:y2},{x:x3,y:y3}];}},addLabelLines:function(pie){var lineGroups=pie.svg.insert("g","."+pie.cssPrefix+"pieChart").attr("class",pie.cssPrefix+"lineGroups").style("opacity",0);var lineGroup=lineGroups.selectAll("."+pie.cssPrefix+"lineGroup").data(pie.lineCoordGroups).enter().append("g").attr("class",pie.cssPrefix+"lineGroup");var lineFunction=d3.svg.line().interpolate("basis").x(function(d){return d.x;}).y(function(d){return d.y;});lineGroup.append("path").attr("d",lineFunction).attr("stroke",function(d,i){return(pie.options.labels.lines.color==="segment")?pie.options.colors[i]:pie.options.labels.lines.color;}).attr("stroke-width",1).attr("fill","none").style("opacity",function(d,i){var percentage=pie.options.labels.outer.hideWhenLessThanPercentage;var segmentPercentage=segments.getPercentage(pie,i);var isHidden=(percentage!==null&&segmentPercentage<percentage)||pie.options.data.content[i].label==="";return isHidden?0:1;});},positionLabelGroups:function(pie,section){d3.selectAll("."+pie.cssPrefix+"labelGroup-"+section).style("opacity",0).attr("transform",function(d,i){var x,y;if(section==="outer"){x=pie.outerLabelGroupData[i].x;y=pie.outerLabelGroupData[i].y;}else{var pieCenterCopy=extend(true,{},pie.pieCenter);if(pie.innerRadius>0){var angle=segments.getSegmentAngle(i,pie.options.data.content,pie.totalSize,{midpoint:true});var newCoords=math.translate(pie.pieCenter.x,pie.pieCenter.y,pie.innerRadius,angle);pieCenterCopy.x=newCoords.x;pieCenterCopy.y=newCoords.y;}
var dims=helpers.getDimensions(pie.cssPrefix+"labelGroup"+i+"-inner");var xOffset=dims.w/2;var yOffset=dims.h/4;x=pieCenterCopy.x+(pie.lineCoordGroups[i][0].x-pieCenterCopy.x)/1.8;y=pieCenterCopy.y+(pie.lineCoordGroups[i][0].y-pieCenterCopy.y)/1.8;x=x-xOffset;y=y+yOffset;}
return"translate("+x+","+y+")";});},fadeInLabelsAndLines:function(pie){var loadSpeed=(pie.options.effects.load.effect==="default")?pie.options.effects.load.speed:1;setTimeout(function(){var labelFadeInTime=(pie.options.effects.load.effect==="default")?400:1;d3.selectAll("."+pie.cssPrefix+"labelGroup-outer").transition().duration(labelFadeInTime).style("opacity",function(d,i){var percentage=pie.options.labels.outer.hideWhenLessThanPercentage;var segmentPercentage=segments.getPercentage(pie,i);return(percentage!==null&&segmentPercentage<percentage)?0:1;});d3.selectAll("."+pie.cssPrefix+"labelGroup-inner").transition().duration(labelFadeInTime).style("opacity",function(d,i){var percentage=pie.options.labels.inner.hideWhenLessThanPercentage;var segmentPercentage=segments.getPercentage(pie,i);return(percentage!==null&&segmentPercentage<percentage)?0:1;});d3.selectAll("g."+pie.cssPrefix+"lineGroups").transition().duration(labelFadeInTime).style("opacity",1);if(helpers.isFunction(pie.options.callbacks.onload)){setTimeout(function(){try{pie.options.callbacks.onload();}catch(e){}},labelFadeInTime);}},loadSpeed);},getIncludes:function(val){var addMainLabel=false;var addValue=false;var addPercentage=false;switch(val){case"label":addMainLabel=true;break;case"value":addValue=true;break;case"percentage":addPercentage=true;break;case"label-value1":case"label-value2":addMainLabel=true;addValue=true;break;case"label-percentage1":case"label-percentage2":addMainLabel=true;addPercentage=true;break;}
return{mainLabel:addMainLabel,value:addValue,percentage:addPercentage};},computeOuterLabelCoords:function(pie){pie.svg.selectAll("."+pie.cssPrefix+"labelGroup-outer").each(function(d,i){return labels.getIdealOuterLabelPositions(pie,i);});labels.resolveOuterLabelCollisions(pie);},resolveOuterLabelCollisions:function(pie){var size=pie.options.data.content.length;labels.checkConflict(pie,0,"clockwise",size);labels.checkConflict(pie,size-1,"anticlockwise",size);},checkConflict:function(pie,currIndex,direction,size){var i,curr;if(size<=1){return;}
var currIndexHemisphere=pie.outerLabelGroupData[currIndex].hs;if(direction==="clockwise"&&currIndexHemisphere!=="right"){return;}
if(direction==="anticlockwise"&&currIndexHemisphere!=="left"){return;}
var nextIndex=(direction==="clockwise")?currIndex+1:currIndex-1;var currLabelGroup=pie.outerLabelGroupData[currIndex];var examinedLabelGroup=pie.outerLabelGroupData[nextIndex];var info={labelHeights:pie.outerLabelGroupData[0].h,center:pie.pieCenter,lineLength:(pie.outerRadius+pie.options.labels.outer.pieDistance),heightChange:pie.outerLabelGroupData[0].h+1};if(direction==="clockwise"){i=0;for(;i<=currIndex;i++){curr=pie.outerLabelGroupData[i];if(helpers.rectIntersect(curr,examinedLabelGroup)){labels.adjustLabelPos(pie,nextIndex,currLabelGroup,info);break;}}}else{i=size-1;for(;i>=currIndex;i--){curr=pie.outerLabelGroupData[i];if(helpers.rectIntersect(curr,examinedLabelGroup)){labels.adjustLabelPos(pie,nextIndex,currLabelGroup,info);break;}}}
labels.checkConflict(pie,nextIndex,direction,size);},adjustLabelPos:function(pie,nextIndex,lastCorrectlyPositionedLabel,info){var xDiff,yDiff,newXPos,newYPos;newYPos=lastCorrectlyPositionedLabel.y+info.heightChange;yDiff=info.center.y-newYPos;if(Math.abs(info.lineLength)>Math.abs(yDiff)){xDiff=Math.sqrt((info.lineLength*info.lineLength)-(yDiff*yDiff));}else{xDiff=Math.sqrt((yDiff*yDiff)-(info.lineLength*info.lineLength));}
if(lastCorrectlyPositionedLabel.hs==="right"){newXPos=info.center.x+xDiff;}else{newXPos=info.center.x-xDiff-pie.outerLabelGroupData[nextIndex].w;}
pie.outerLabelGroupData[nextIndex].x=newXPos;pie.outerLabelGroupData[nextIndex].y=newYPos;},getIdealOuterLabelPositions:function(pie,i){var labelGroupDims=d3.select("#"+pie.cssPrefix+"labelGroup"+i+"-outer").node().getBBox();var angle=segments.getSegmentAngle(i,pie.options.data.content,pie.totalSize,{midpoint:true});var originalX=pie.pieCenter.x;var originalY=pie.pieCenter.y-(pie.outerRadius+pie.options.labels.outer.pieDistance);var newCoords=math.rotate(originalX,originalY,pie.pieCenter.x,pie.pieCenter.y,angle);var hemisphere="right";if(angle>180){newCoords.x-=(labelGroupDims.width+8);hemisphere="left";}else{newCoords.x+=8;}
pie.outerLabelGroupData[i]={x:newCoords.x,y:newCoords.y,w:labelGroupDims.width,h:labelGroupDims.height,hs:hemisphere};}};var segments={create:function(pie){var pieCenter=pie.pieCenter;var colors=pie.options.colors;var loadEffects=pie.options.effects.load;var segmentStroke=pie.options.misc.colors.segmentStroke;var pieChartElement=pie.svg.insert("g","#"+pie.cssPrefix+"title").attr("transform",function(){return math.getPieTranslateCenter(pieCenter);}).attr("class",pie.cssPrefix+"pieChart");var arc=d3.svg.arc().innerRadius(pie.innerRadius).outerRadius(pie.outerRadius).startAngle(0).endAngle(function(d){return(d.value/pie.totalSize)*2*Math.PI;});var g=pieChartElement.selectAll("."+pie.cssPrefix+"arc").data(pie.options.data.content).enter().append("g").attr("class",pie.cssPrefix+"arc");var loadSpeed=loadEffects.speed;if(loadEffects.effect==="none"){loadSpeed=0;}
g.append("path").attr("id",function(d,i){return pie.cssPrefix+"segment"+i;}).attr("fill",function(d,i){var color=colors[i];if(pie.options.misc.gradient.enabled){color="url(#"+pie.cssPrefix+"grad"+i+")";}
return color;}).style("stroke",segmentStroke).style("stroke-width",1).transition().ease("cubic-in-out").duration(loadSpeed).attr("data-index",function(d,i){return i;}).attrTween("d",function(b){var i=d3.interpolate({value:0},b);return function(t){return pie.arc(i(t));};});pie.svg.selectAll("g."+pie.cssPrefix+"arc").attr("transform",function(d,i){var angle=0;if(i>0){angle=segments.getSegmentAngle(i-1,pie.options.data.content,pie.totalSize);}
return"rotate("+angle+")";});pie.arc=arc;},addGradients:function(pie){var grads=pie.svg.append("defs").selectAll("radialGradient").data(pie.options.data.content).enter().append("radialGradient").attr("gradientUnits","userSpaceOnUse").attr("cx",0).attr("cy",0).attr("r","120%").attr("id",function(d,i){return pie.cssPrefix+"grad"+i;});grads.append("stop").attr("offset","0%").style("stop-color",function(d,i){return pie.options.colors[i];});grads.append("stop").attr("offset",pie.options.misc.gradient.percentage+"%").style("stop-color",pie.options.misc.gradient.color);},addSegmentEventHandlers:function(pie){var arc=d3.selectAll("."+pie.cssPrefix+"arc,."+pie.cssPrefix+"labelGroup-inner,."+pie.cssPrefix+"labelGroup-outer");arc.on("click",function(){var currentEl=d3.select(this);var segment;if(currentEl.attr("class")===pie.cssPrefix+"arc"){segment=currentEl.select("path");}else{var index=currentEl.attr("data-index");segment=d3.select("#"+pie.cssPrefix+"segment"+index);}
var isExpanded=segment.attr("class")===pie.cssPrefix+"expanded";segments.onSegmentEvent(pie,pie.options.callbacks.onClickSegment,segment,isExpanded);if(pie.options.effects.pullOutSegmentOnClick.effect!=="none"){if(isExpanded){segments.closeSegment(pie,segment.node());}else{segments.openSegment(pie,segment.node());}}});arc.on("mouseover",function(){var currentEl=d3.select(this);var segment,index;if(currentEl.attr("class")===pie.cssPrefix+"arc"){segment=currentEl.select("path");}else{index=currentEl.attr("data-index");segment=d3.select("#"+pie.cssPrefix+"segment"+index);}
if(pie.options.effects.highlightSegmentOnMouseover){index=segment.attr("data-index");var segColor=pie.options.colors[index];segment.style("fill",helpers.getColorShade(segColor,pie.options.effects.highlightLuminosity));}
if(pie.options.tooltips.enabled){index=segment.attr("data-index");tt.showTooltip(pie,index);}
var isExpanded=segment.attr("class")===pie.cssPrefix+"expanded";segments.onSegmentEvent(pie,pie.options.callbacks.onMouseoverSegment,segment,isExpanded);});arc.on("mousemove",function(){tt.moveTooltip(pie);});arc.on("mouseout",function(){var currentEl=d3.select(this);var segment,index;if(currentEl.attr("class")===pie.cssPrefix+"arc"){segment=currentEl.select("path");}else{index=currentEl.attr("data-index");segment=d3.select("#"+pie.cssPrefix+"segment"+index);}
if(pie.options.effects.highlightSegmentOnMouseover){index=segment.attr("data-index");var color=pie.options.colors[index];if(pie.options.misc.gradient.enabled){color="url(#"+pie.cssPrefix+"grad"+index+")";}
segment.style("fill",color);}
if(pie.options.tooltips.enabled){index=segment.attr("data-index");tt.hideTooltip(pie,index);}
var isExpanded=segment.attr("class")===pie.cssPrefix+"expanded";segments.onSegmentEvent(pie,pie.options.callbacks.onMouseoutSegment,segment,isExpanded);});},onSegmentEvent:function(pie,func,segment,isExpanded){if(!helpers.isFunction(func)){return;}
var index=parseInt(segment.attr("data-index"),10);func({segment:segment.node(),index:index,expanded:isExpanded,data:pie.options.data.content[index]});},openSegment:function(pie,segment){if(pie.isOpeningSegment){return;}
pie.isOpeningSegment=true;if(d3.selectAll("."+pie.cssPrefix+"expanded").length>0){segments.closeSegment(pie,d3.select("."+pie.cssPrefix+"expanded").node());}
d3.select(segment).transition().ease(pie.options.effects.pullOutSegmentOnClick.effect).duration(pie.options.effects.pullOutSegmentOnClick.speed).attr("transform",function(d,i){var c=pie.arc.centroid(d),x=c[0],y=c[1],h=Math.sqrt(x*x+y*y),pullOutSize=parseInt(pie.options.effects.pullOutSegmentOnClick.size,10);return"translate("+((x/h)*pullOutSize)+','+((y/h)*pullOutSize)+")";}).each("end",function(d,i){pie.currentlyOpenSegment=segment;pie.isOpeningSegment=false;d3.select(this).attr("class",pie.cssPrefix+"expanded");});},closeSegment:function(pie,segment){d3.select(segment).transition().duration(400).attr("transform","translate(0,0)").each("end",function(d,i){d3.select(this).attr("class","");pie.currentlyOpenSegment=null;});},getCentroid:function(el){var bbox=el.getBBox();return{x:bbox.x+bbox.width/2,y:bbox.y+bbox.height/2};},getSegmentAngle:function(index,data,totalSize,opts){var options=extend({compounded:true,midpoint:false},opts);var currValue=data[index].value;var fullValue;if(options.compounded){fullValue=0;for(var i=0;i<=index;i++){fullValue+=data[i].value;}}
if(typeof fullValue==='undefined'){fullValue=currValue;}
var angle=(fullValue/totalSize)*360;if(options.midpoint){var currAngle=(currValue/totalSize)*360;angle-=(currAngle/2);}
return angle;},getPercentage:function(pie,index){return Math.floor((pie.options.data.content[index].value/pie.totalSize)*100);}};var text={offscreenCoord:-10000,addTitle:function(pie){var title=pie.svg.selectAll("."+pie.cssPrefix+"title").data([pie.options.header.title]).enter().append("text").text(function(d){return d.text;}).attr({id:pie.cssPrefix+"title",class:pie.cssPrefix+"title",x:text.offscreenCoord,y:text.offscreenCoord}).attr("text-anchor",function(){var location;if(pie.options.header.location==="top-center"||pie.options.header.location==="pie-center"){location="middle";}else{location="left";}
return location;}).attr("fill",function(d){return d.color;}).style("font-size",function(d){return d.fontSize+"px";}).style("font-family",function(d){return d.font;});},positionTitle:function(pie){var textComponents=pie.textComponents;var headerLocation=pie.options.header.location;var canvasPadding=pie.options.misc.canvasPadding;var canvasWidth=pie.options.size.canvasWidth;var titleSubtitlePadding=pie.options.header.titleSubtitlePadding;var x;if(headerLocation==="top-left"){x=canvasPadding.left;}else{x=((canvasWidth-canvasPadding.right)/2)+canvasPadding.left;}
x+=pie.options.misc.pieCenterOffset.x;var y=canvasPadding.top+textComponents.title.h;if(headerLocation==="pie-center"){y=pie.pieCenter.y;if(textComponents.subtitle.exists){var totalTitleHeight=textComponents.title.h+titleSubtitlePadding+textComponents.subtitle.h;y=y-(totalTitleHeight/2)+textComponents.title.h;}else{y+=(textComponents.title.h/4);}}
pie.svg.select("#"+pie.cssPrefix+"title").attr("x",x).attr("y",y);},addSubtitle:function(pie){var headerLocation=pie.options.header.location;pie.svg.selectAll("."+pie.cssPrefix+"subtitle").data([pie.options.header.subtitle]).enter().append("text").text(function(d){return d.text;}).attr("x",text.offscreenCoord).attr("y",text.offscreenCoord).attr("id",pie.cssPrefix+"subtitle").attr("class",pie.cssPrefix+"subtitle").attr("text-anchor",function(){var location;if(headerLocation==="top-center"||headerLocation==="pie-center"){location="middle";}else{location="left";}
...
return h;}};var tt={addTooltips:function(pie){var tooltips=pie.svg.insert("g").attr("class",pie.cssPrefix+"tooltips");tooltips.selectAll("."+pie.cssPrefix+"tooltip").data(pie.options.data.content).enter().append("g").attr("class",pie.cssPrefix+"tooltip").attr("id",function(d,i){return pie.cssPrefix+"tooltip"+i;}).style("opacity",0).append("rect").attr({rx:pie.options.tooltips.styles.borderRadius,ry:pie.options.tooltips.styles.borderRadius,x:-pie.options.tooltips.styles.padding,opacity:pie.options.tooltips.styles.backgroundOpacity}).style("fill",pie.options.tooltips.styles.backgroundColor);tooltips.selectAll("."+pie.cssPrefix+"tooltip").data(pie.options.data.content).append("text").attr("fill",function(d){return pie.options.tooltips.styles.color;}).style("font-size",function(d){return pie.options.tooltips.styles.fontSize;}).style("font-family",function(d){return pie.options.tooltips.styles.font;}).text(function(d,i){var caption=pie.options.tooltips.string;if(pie.options.tooltips.type==="caption"){caption=d.caption;}
return tt.replacePlaceholders(pie,caption,i,{label:d.label,value:d.value,percentage:segments.getPercentage(pie,i)});});tooltips.selectAll("."+pie.cssPrefix+"tooltip rect").attr({width:function(d,i){var dims=helpers.getDimensions(pie.cssPrefix+"tooltip"+i);return dims.w+(2*pie.options.tooltips.styles.padding);},height:function(d,i){var dims=helpers.getDimensions(pie.cssPrefix+"tooltip"+i);return dims.h+(2*pie.options.tooltips.styles.padding);},y:function(d,i){var dims=helpers.getDimensions(pie.cssPrefix+"tooltip"+i);return-(dims.h/2)+1;}});},showTooltip:function(pie,index){var fadeInSpeed=pie.options.tooltips.styles.fadeInSpeed;if(tt.currentTooltip===index){fadeInSpeed=1;}
tt.currentTooltip=index;d3.select("#"+pie.cssPrefix+"tooltip"+index).transition().duration(fadeInSpeed).style("opacity",function(){return 1;});tt.moveTooltip(pie);},moveTooltip:function(pie){d3.selectAll("#"+pie.cssPrefix+"tooltip"+tt.currentTooltip).attr("transform",function(d){var mouseCoords=d3.mouse(this.parentElement);var x=mouseCoords[0]+pie.options.tooltips.styles.padding+2;var y=mouseCoords[1]-(2*pie.options.tooltips.styles.padding)-2;return"translate("+x+","+y+")";});},hideTooltip:function(pie,index){d3.select("#"+pie.cssPrefix+"tooltip"+index).style("opacity",function(){return 0;});d3.select("#"+pie.cssPrefix+"tooltip"+tt.currentTooltip).attr("transform",function(d,i){var x=pie.options.size.canvasWidth+1000;var y=pie.options.size.canvasHeight+1000;return"translate("+x+","+y+")";});},replacePlaceholders:function(pie,str,index,replacements){if(helpers.isFunction(pie.options.tooltips.placeholderParser)){pie.options.tooltips.placeholderParser(index,replacements);}
var replacer=function(){return function(match){var placeholder=arguments[1];if(replacements.hasOwnProperty(placeholder)){return replacements[arguments[1]];}else{return arguments[0];}};};return str.replace(/\{(\w+)\}/g,replacer(replacements));}};var d3pie=function(element,options){this.element=element;if(typeof element==="string"){var el=element.replace(/^#/,"");this.element=document.getElementById(el);}
var opts={};extend(true,opts,defaultSettings,options);this.options=opts;if(this.options.misc.cssPrefix!==null){this.cssPrefix=this.options.misc.cssPrefix;}else{this.cssPrefix="p"+_uniqueIDCounter+"_";_uniqueIDCounter++;}
if(!validate.initialCheck(this)){return;}
d3.select(this.element).attr(_scriptName,_version);this.options.data.content=math.sortPieData(this);if(this.options.data.smallSegmentGrouping.enabled){this.options.data.content=helpers.applySmallSegmentGrouping(this.options.data.content,this.options.data.smallSegmentGrouping);}
this.options.colors=helpers.initSegmentColors(this);this.totalSize=math.getTotalPieSize(this.options.data.content);_init.call(this);};d3pie.prototype.recreate=function(){this.options.data.content=math.sortPieData(this);if(this.options.data.smallSegmentGrouping.enabled){this.options.data.content=helpers.applySmallSegmentGrouping(this.options.data.content,this.options.data.smallSegmentGrouping);}
this.options.colors=helpers.initSegmentColors(this);this.totalSize=math.getTotalPieSize(this.options.data.content);_init.call(this);};d3pie.prototype.redraw=function(){this.element.innerHTML="";_init.call(this);};d3pie.prototype.destroy=function(){this.element.innerHTML="";d3.select(this.element).attr(_scriptName,null);};d3pie.prototype.getOpenSegment=function(){var segment=this.currentlyOpenSegment;if(segment!==null&&typeof segment!=="undefined"){var index=parseInt(d3.select(segment).attr("data-index"),10);return{element:segment,index:index,data:this.options.data.content[index]};}else{return null;}};d3pie.prototype.openSegment=function(index){index=parseInt(index,10);if(index<0||index>this.options.data.content.length-1){return;}
segments.openSegment(this,d3.select("#"+this.cssPrefix+"segment"+index).node());};d3pie.prototype.closeSegment=function(){var segment=this.currentlyOpenSegment;if(segment){segments.closeSegment(this,segment);}};d3pie.prototype.updateProp=function(propKey,value){switch(propKey){case"header.title.text":var oldVal=helpers.processObj(this.options,propKey);helpers.processObj(this.options,propKey,value);d3.select("#"+this.cssPrefix+"title").html(value);if((oldVal===""&&value!=="")||(oldVal!==""&&value==="")){this.redraw();}
break;case"header.subtitle.text":var oldValue=helpers.processObj(this.options,propKey);helpers.processObj(this.options,propKey,value);d3.select("#"+this.cssPrefix+"subtitle").html(value);if((oldValue===""&&value!=="")||(oldValue!==""&&value==="")){this.redraw();}
break;case"callbacks.onload":case"callbacks.onMouseoverSegment":case"callbacks.onMouseoutSegment":case"callbacks.onClickSegment":case"effects.pullOutSegmentOnClick.effect":case"effects.pullOutSegmentOnClick.speed":case"effects.pullOutSegmentOnClick.size":case"effects.highlightSegmentOnMouseover":case"effects.highlightLuminosity":helpers.processObj(this.options,propKey,value);break;default:helpers.processObj(this.options,propKey,value);this.destroy();this.recreate();break;}};var _init=function(){this.svg=helpers.addSVGSpace(this);this.textComponents={headerHeight:0,title:{exists:this.options.header.title.text!=="",h:0,w:0},subtitle:{exists:this.options.header.subtitle.text!=="",h:0,w:0},footer:{exists:this.options.footer.text!=="",h:0,w:0}};this.outerLabelGroupData=[];if(this.textComponents.title.exists){text.addTitle(this);}
if(this.textComponents.subtitle.exists){text.addSubtitle(this);}
text.addFooter(this);var self=this;helpers.whenIdExists(this.cssPrefix+"footer",function(){text.positionFooter(self);var d3=helpers.getDimensions(self.cssPrefix+"footer");self.textComponents.footer.h=d3.h;self.textComponents.footer.w=d3.w;});var reqEls=[];if(this.textComponents.title.exists){reqEls.push(this.cssPrefix+"title");}
if(this.textComponents.subtitle.exists){reqEls.push(this.cssPrefix+"subtitle");}
if(this.textComponents.footer.exists){reqEls.push(this.cssPrefix+"footer");}
helpers.whenElementsExist(reqEls,function(){if(self.textComponents.title.exists){var d1=helpers.getDimensions(self.cssPrefix+"title");self.textComponents.title.h=d1.h;self.textComponents.title.w=d1.w;}
if(self.textComponents.subtitle.exists){var d2=helpers.getDimensions(self.cssPrefix+"subtitle");self.textComponents.subtitle.h=d2.h;self.textComponents.subtitle.w=d2.w;}
if(self.textComponents.title.exists||self.textComponents.subtitle.exists){var headerHeight=0;if(self.textComponents.title.exists){headerHeight+=self.textComponents.title.h;if(self.textComponents.subtitle.exists){headerHeight+=self.options.header.titleSubtitlePadding;}}
if(self.textComponents.subtitle.exists){headerHeight+=self.textComponents.subtitle.h;}
self.textComponents.headerHeight=headerHeight;}
math.computePieRadius(self);math.calculatePieCenter(self);text.positionTitle(self);text.positionSubtitle(self);if(self.options.misc.gradient.enabled){segments.addGradients(self);}
segments.create(self);labels.add(self,"inner",self.options.labels.inner.format);labels.add(self,"outer",self.options.labels.outer.format);labels.positionLabelElements(self,"inner",self.options.labels.inner.format);labels.positionLabelElements(self,"outer",self.options.labels.outer.format);labels.computeOuterLabelCoords(self);labels.positionLabelGroups(self,"outer");labels.computeLabelLinePositions(self);if(self.options.labels.lines.enabled&&self.options.labels.outer.format!=="none"){labels.addLabelLines(self);}
labels.positionLabelGroups(self,"inner");labels.fadeInLabelsAndLines(self);if(self.options.tooltips.enabled){tt.addTooltips(self);}
segments.addSegmentEventHandlers(self);});};return d3pie;}));(function(definition){if(typeof bootstrap==="function"){bootstrap("promise",definition);}else if(typeof exports==="object"){module.exports=definition();}else if(typeof define==="function"&&define.amd){define(definition);}else if(typeof ses!=="undefined"){if(!ses.ok()){return;}else{ses.makeQ=definition;}}else{Q=definition();}})(function(){"use strict";var hasStacks=false;try{throw new Error();}catch(e){hasStacks=!!e.stack;}
...
Promise.prototype.nodeify=function(nodeback){if(nodeback){this.then(function(value){nextTick(function(){nodeback(null,value);});},function(error){nextTick(function(){nodeback(error);});});}else{return this;}};var qEndingLine=captureLine();return Q;});;(function(){if(typeof module==='object'&&module.exports){module.exports=LRUCache}else{this.LRUCache=LRUCache}
function hOP(obj,key){return Object.prototype.hasOwnProperty.call(obj,key)}
function naiveLength(){return 1}
function LRUCache(options){if(!(this instanceof LRUCache))
...
this.reset()}
Object.defineProperty(LRUCache.prototype,"max",{set:function(mL){if(!mL||!(typeof mL==="number")||mL<=0)mL=Infinity
this._max=mL
if(this._length>this._max)trim(this)},get:function(){return this._max},enumerable:true})
Object.defineProperty(LRUCache.prototype,"lengthCalculator",{set:function(lC){if(typeof lC!=="function"){this._lengthCalculator=naiveLength
this._length=this._itemCount
for(var key in this._cache){this._cache[key].length=1}}else{this._lengthCalculator=lC
this._length=0
for(var key in this._cache){this._cache[key].length=this._lengthCalculator(this._cache[key].value)
this._length+=this._cache[key].length}}
if(this._length>this._max)trim(this)},get:function(){return this._lengthCalculator},enumerable:true})
Object.defineProperty(LRUCache.prototype,"length",{get:function(){return this._length},enumerable:true})
Object.defineProperty(LRUCache.prototype,"itemCount",{get:function(){return this._itemCount},enumerable:true})
LRUCache.prototype.forEach=function(fn,thisp){thisp=thisp||this
var i=0;for(var k=this._mru-1;k>=0&&i<this._itemCount;k--)if(this._lruList[k]){i++
var hit=this._lruList[k]
if(this._maxAge&&(Date.now()-hit.now>this._maxAge)){del(this,hit)
if(!this._allowStale)hit=undefined}
if(hit){fn.call(thisp,hit.value,hit.key,this)}}}
LRUCache.prototype.keys=function(){var keys=new Array(this._itemCount)
var i=0
for(var k=this._mru-1;k>=0&&i<this._itemCount;k--)if(this._lruList[k]){var hit=this._lruList[k]
keys[i++]=hit.key}
return keys}
LRUCache.prototype.values=function(){var values=new Array(this._itemCount)
var i=0
for(var k=this._mru-1;k>=0&&i<this._itemCount;k--)if(this._lruList[k]){var hit=this._lruList[k]
values[i++]=hit.value}
return values}
LRUCache.prototype.reset=function(){if(this._dispose&&this._cache){for(var k in this._cache){this._dispose(k,this._cache[k].value)}}
...
this._length=0
this._itemCount=0}
LRUCache.prototype.dump=function(){return this._cache}
LRUCache.prototype.dumpLru=function(){return this._lruList}
LRUCache.prototype.set=function(key,value){if(hOP(this._cache,key)){if(this._dispose)this._dispose(key,this._cache[key].value)
...
if(hit.length>this._max){if(this._dispose)this._dispose(key,value)
return false}
...
return true}
LRUCache.prototype.has=function(key){if(!hOP(this._cache,key))return false
var hit=this._cache[key]
if(this._maxAge&&(Date.now()-hit.now>this._maxAge)){return false}
return true}
LRUCache.prototype.get=function(key){return get(this,key,true)}
LRUCache.prototype.peek=function(key){return get(this,key,false)}
LRUCache.prototype.pop=function(){var hit=this._lruList[this._lru]
del(this,hit)
return hit||null}
LRUCache.prototype.del=function(key){del(this,this._cache[key])}
function get(self,key,doUse){var hit=self._cache[key]
if(hit){if(self._maxAge&&(Date.now()-hit.now>self._maxAge)){del(self,hit)
if(!self._allowStale)hit=undefined}else{if(doUse)use(self,hit)}
if(hit)hit=hit.value}
return hit}
function use(self,hit){shiftLU(self,hit)
hit.lu=self._mru++
self._lruList[hit.lu]=hit}
function trim(self){while(self._lru<self._mru&&self._length>self._max)
del(self,self._lruList[self._lru])}
function shiftLU(self,hit){delete self._lruList[hit.lu]
while(self._lru<self._mru&&!self._lruList[self._lru])self._lru++}
function del(self,hit){if(hit){if(self._dispose)self._dispose(hit.key,hit.value)
self._length-=hit.length
self._itemCount--
delete self._cache[hit.key]
shiftLU(self,hit)}}
function Entry(key,value,lu,length,now){this.key=key
this.value=value
this.lu=lu
this.length=length
this.now=now}})()
var Util={};Util.ObjConstrunctor={}.constructor;var require=function(key){if(key=="ApplaneCore/apputil/util.js"){return Util;}else if(key=="q"){return Q;}else if(key=="../../lib/ModuleManager.js"){return ModuleManager;}else if(key=="ApplaneDB/lib/EventManager.js"){return EventManager;}else if(key=="ApplaneDB/lib/Document.js"){return Document;}else if(key=="./DataModel.js"){return DataModel;}else if(key=="ApplaneDB/public/js/ViewUtility.js"){return ViewUtility;}else{alert("Not supported required>>>"+key)}};Util.NOT_CONNECTED_MESSAGE="Unable to connect. Check your internet connection."
Util.INVALID_TOKEN="Not connected"
Util.UNEXPECTED_ERROR="Something went wrong, but we're working to fix it as soon as we can. Thank you for your patience.";Util.clone=function(value){return angular.copy(value);}
Util.isObject=function(obj){if(obj===undefined||obj===null||obj===true||obj===false||Array.isArray(obj)){return false;}else if(obj.constructor===Util.ObjConstrunctor){return true;}else{return false;}}
Util.isArray=function(value){return angular.isArray(value);}
Util.equals=function(value1,value2){return angular.equals(value1,value2);}
Util.getIndex=function(array,field,value){if(!array||array.length==0){return undefined;}
for(var i=0;i<array.length;i++){if(array[i][field]==value){return i;}}}
Util.sort=function(data,type,field){if(!data||data.length==0){return;}
if(!field){if(!type||type=="asc"){data.sort(function(a,b){return a-b});}else if(type=="desc"){data.sort(function(a,b){return a-b});data.reverse();}}else{var fieldValueType=undefined;for(var i=0;i<data.length;i++){if(data[i][field]){var fieldValue=data[i][field];fieldValueType=typeof fieldValue;if(fieldValueType==="object"&&fieldValue instanceof Date){fieldValueType="date";}
break;}}
if(fieldValueType){if(!type||type=="asc"){data.sort(function(a,b){if(a[field]===undefined&&b[field]===undefined)
return 0;if(a[field]===undefined&&b[field]!==undefined)
return-1;if(a[field]!==undefined&&b[field]===undefined)
return 1;if(fieldValueType==="number"){return a[field]-b[field];}else if(fieldValueType==="string"){var nameA=a[field].toLowerCase();var nameB=b[field].toLowerCase();if(nameA<nameB)
return-1
if(nameA>nameB)
return 1
return 0}else if(fieldValueType==="date"){var dateA=new Date(a[field]);var dateB=new Date(b[field]);return dateA-dateB;}})}else if(type=="desc"){data.sort(function(a,b){if(a[field]===undefined&&b[field]===undefined)
return 0;if(a[field]===undefined&&b[field]!==undefined)
return 1;if(a[field]!==undefined&&b[field]===undefined)
return-1;if(fieldValueType==="number"){return b[field]-a[field];}else if(fieldValueType==="string"){var nameA=a[field].toLowerCase();var nameB=b[field].toLowerCase();if(nameB<nameA)
return-1
if(nameB>nameA)
return 1
return 0}else if(fieldValueType==="date"){var dateA=new Date(a[field]);var dateB=new Date(b[field])
return dateB-dateA;}})}}}}
Util.isJSONObject=function(value){return Util.isObject(value)}
Util.deepEqual=function(val1,val2){return Util.equals(val1,val2);}
Util.deepClone=function(value){return angular.copy(value);}
Util.iterateArrayWithPromise=function(array,task){var Q=require("q");var D=Q.defer();var index=0;var loop=function(index){try{var onResolve=function(){index=index+1;if(index==array.length){D.resolve();}else{loop(index);}}
try{var p=task(index,array[index]);if(!p){onResolve();return;}
p.then(onResolve).fail(function(err){D.reject(err);})}catch(e){D.reject(e);}}catch(e){D.reject(e);}}
if(!array||array.length==0){D.resolve();}else{loop(0);}
return D.promise;}
Util.iterateArrayWithoutPromise=function(array,task,reverse){var length=array?array.length:0;if(length==0){return;}
var index=reverse?(length-1):0;function loop(index){try{var onResolve=function(){index=reverse?(index-1):index+1;if((!reverse&&index==array.length)||(reverse&&index==-1)){return;}else{return loop(index);}}
try{var p=task(index,array[index]);if(Q.isPromise(p)){return p.then(onResolve).fail(function(err){throw err;})}else{return onResolve();}}catch(e){throw e;}}catch(e){throw e;}}
return loop(index);}
Util.getUniqueTempId=function(){Util.nextUniqueTemp=Util.nextUniqueTemp||0;Util.nextUniqueTemp=Util.nextUniqueTemp+1;return Util.nextUniqueTemp+"__temp__";}
Util.isTemp=function(_id){if((!_id)||(typeof _id=='string'&&_id.indexOf("__temp__")>0)){return true;}}
Util.populate_IdInArray=function(updates){if(!updates){return;}
for(var k in updates){var value=updates[k];if(Array.isArray(value)){for(var i=0;i<value.length;i++){if(Util.isJSONObject(value[i])){if(!(value[i]._id)){value[i]._id=Util.getUniqueTempId();}
Util.populate_IdInArray(value[i]);}else{break;}}}else if(Util.isJSONObject(value)){Util.populate_IdInArray(value);}}}
Util.isExists=function(array,value,key){if(!array||!value){return;}
return Util.checkValue(array,value,key);};Util.checkValue=function(array,value,key){for(var i=0;i<array.length;i++){var cExp;var exp;if(key&&Util.isJSONObject(value)){cExp=array[i][key];exp=value[key];}else{cExp=array[i];exp=value;}
if(Util.deepEqual(cExp,exp)){return i;}}}
Util.putDottedValue=function(model,expression,value,populateTempId){if(!model){alert("Model does not exits for putting dotted value")
return;}
var lastDottedIndex=expression.lastIndexOf(".");if(lastDottedIndex>=0){var firstExpression=expression.substring(0,lastDottedIndex);expression=expression.substring(lastDottedIndex+1);model=Util.resolveDot(model,firstExpression,true);}
if(Array.isArray(model)){if(model.length==0){var newObject={}
if(populateTempId){newObject._id=Util.getUniqueTempId()}
model.push(newObject);}
var model=model[0];model[expression]=value;}else if(Util.isJSONObject(model)){model[expression]=value;}else{throw new Error("Un supported model while putDottedValue>>>>"+JSON.stringify(model)+">>>>expression>>>>"+expression+">>value>>>>"+JSON.stringify(value));}}
Util.resolveDottedValue=function(model,expression,confirm,confirmType){return Util.resolveDot(model,expression,confirm,confirmType);}
Util.resolveDot=function(model,expression,confirm,confirmType){if(!model){return;}
while(expression!==undefined){var fieldIndex=expression.indexOf(".");var exp=expression;if(fieldIndex>=0){exp=expression.substring(0,fieldIndex);expression=expression.substring(fieldIndex+1);}else{expression=undefined;}
if((model[exp]===undefined||model[exp]===null)&&!confirm){return;}
if(model[exp]!==undefined&&model[exp]!==null){model=model[exp];}else{if(expression){model[exp]={}}else{if(confirmType=='array'){model[exp]=[];}else{model[exp]={}}}
model=model[exp];}}
return model;}
Util.evaluateFilter=function(filter,row){var keys=Object.keys(filter);var keyLength=keys.length;var evaluated=false;for(var i=0;i<keyLength;i++){var key=keys[i];if(key==="$or"||key==="$and"){var value=filter[key];if(value){for(var j=0;j<value.length;j++){evaluated=Util.evaluateFilter(value[j],row);if((evaluated&&key==="$or")||(!evaluated&&key==="$and")){break;}}}else{evaluated=true;}}else{if(Util.isJSONObject(filter[key])){for(var opr in filter[key]){evaluated=evaluateExpression(key,opr,filter[key][opr],row);if(!evaluated){break;}}}else{evaluated=evaluateExpression(key,"$eq",filter[key],row);}}
if(!evaluated){break;}}
return evaluated;}
function evaluateExpression(column,operator,filterValue,row){var columnValue=Util.resolveValue(row,column);if(operator=="$eq"){return((columnValue===null||columnValue===undefined)&&(filterValue===null||filterValue===undefined))||Util.deepEqual(columnValue,filterValue);}
return false;}
Util.resolveValue=function(row,expression){return Util.resolveDot(row,expression);}
Util.getField=function(expression,fields){if(fields&&fields.length>0){var index=expression.indexOf(".");if(index!==-1){var firstPart=expression.substr(0,index);var rest=expression.substr(index+1);var firstPartField=getFieldDef(firstPart,fields);if(firstPartField){return Util.getField(rest,firstPartField.fields);}}else{return getFieldDef(expression,fields);}}}
Util.replaceDotToUnderscore=function(value){if(value&&value.indexOf(".")>0){return value.replace(/\./g,"_");}else{return value;}}
function getFieldDef(expression,fields){for(var i=0;i<fields.length;i++){var field=fields[i];if(field.field===expression){return field;}}}
function setPlaneData(data,newData,alias){if(data&&data.length>0){for(var i=0;i<data.length;i++){var record=data[i];newData.push(record);if(record[alias]&&record[alias].length>0){setPlaneData(record[alias],newData,alias)}}}}
Util.populateDataKeyMapping=function(data,dataModel){dataModel.keyMapping={};if(data===undefined||dataModel===undefined||!(Array.isArray(data))){return;}
var _id=undefined;for(var i=0;i<data.length;i++){_id=data[i]._id;if(angular.isObject(_id)){_id=JSON.stringify(_id);}
if(dataModel.keyMapping[_id]!==undefined){continue;}
dataModel.keyMapping[_id]=i;}};Util.getDataMappingKey=function(entity,keyMapping){if(entity==undefined||entity._id==undefined||keyMapping===undefined){return undefined;}else{var _id=entity._id;if(angular.isObject(_id)){_id=JSON.stringify(_id);}
return keyMapping[_id];}};Util.validateData=function(data,query,metadata,dataModel){var viewFields=metadata.viewFields;if(viewFields&&viewFields.length>0){var viewField=viewFields[0];if(data&&data.length>1){throw new Error("data length should be equal to 1 when viewFields are defined");}
if(data&&data.length>0){var _id=data[0]["_id"];var viewFieldData=data[0][viewField];metadata._id=_id;data.splice(0,(data.length));for(var i=0;i<viewFieldData.length;i++){data.push(viewFieldData[i]);}}}
var alias=query&&query.$recursion&&query.$recursion.$alias?query.$recursion.$alias:"children";ensure_IdInArray(data,alias);if(metadata.transformRecursionToLinear){var newData=[];setPlaneData(data,newData,alias);dataModel.data=newData;}
if(data&&data.length>0){for(var i=0;i<data.length;i++){if(!data[i]._id){data[i]._id=Util.getUniqueTempId();}}}
if(!metadata.dataProcessedOnServer){return ViewUtility.processDataForView(data,query,metadata);}else{metadata.dataProcessedOnServer=false;}}
Util.setDateWithZeroTimezone=function(dateToSet){var utcDate=new Date("2014-01-10");utcDate.setUTCFullYear(dateToSet.getFullYear());utcDate.setUTCMonth(dateToSet.getMonth());utcDate.setUTCDate(dateToSet.getDate());utcDate.setUTCHours(0);utcDate.setUTCMinutes(0);utcDate.setUTCSeconds(0);utcDate.setUTCMilliseconds(0);return utcDate;}
function ensure_IdInArray(data,alias){for(var i=0;i<data.length;i++){var row=data[i];Util.populate_IdInArray(row);if(row[alias]&&row[alias].length>0){ensure_IdInArray(row[alias],alias);}}}
Util.INC_VALUE=100;function handleDown(array,sortField,srcIndex,targetIndex){var updatedFields=[];var targetField=array[targetIndex];var targetFieldSortOrder=targetField[sortField];targetFieldSortOrder=targetFieldSortOrder;var sourceField=array[srcIndex];if(targetIndex==(array.length-1)){sourceField[sortField]=targetFieldSortOrder+Util.INC_VALUE;updatedFields.push(sourceField);}else{var targetNextIndex=targetIndex+1;var targetNextField=array[targetNextIndex];if(targetNextField.__systemcolumn__){sourceField[sortField]=targetFieldSortOrder+Util.INC_VALUE;updatedFields.push(sourceField);}else{var targetNextFieldSortOrder=targetNextField[sortField];targetNextFieldSortOrder=targetNextFieldSortOrder;if(targetFieldSortOrder<targetNextFieldSortOrder){var newValue=(targetFieldSortOrder+targetNextFieldSortOrder)/2;sourceField[sortField]=newValue;updatedFields.push(sourceField);}else if(targetFieldSortOrder>targetNextFieldSortOrder){throw new Error("Target next index can never be less than target index")}else{var counter=2;var higherValue=undefined;var lastIndex=undefined;for(var i=targetIndex+1;i<array.length;i++){var iteratorField=array[i];if(iteratorField.__systemcolumn__){break;}
var iteratorFieldSortOrder=iteratorField[sortField];iteratorFieldSortOrder=iteratorFieldSortOrder;if(iteratorFieldSortOrder>targetFieldSortOrder){higherValue=iteratorFieldSortOrder;lastIndex=i-1;break;}else if(iteratorFieldSortOrder<targetFieldSortOrder){throw new Error("Target next index can never be less than target index")}else{lastIndex=i;counter+=1;}}
if(higherValue==undefined){higherValue=targetFieldSortOrder+Util.INC_VALUE;}
var difference=(higherValue-targetFieldSortOrder)/counter;var cumulativeDiff=difference+targetFieldSortOrder;sourceField[sortField]=cumulativeDiff;updatedFields.push(sourceField);for(var i=targetIndex+1;i<=lastIndex;i++){cumulativeDiff+=difference;array[i][sortField]=cumulativeDiff;updatedFields.push(array[i]);}}}}
return updatedFields;}
function handleUp(array,sortField,srcIndex,targetIndex){var updatedFields=[];var targetField=array[targetIndex];var targetFieldSortOrder=targetField[sortField];targetFieldSortOrder=targetFieldSortOrder;var sourceField=array[srcIndex];if(targetIndex==0){sourceField[sortField]=targetFieldSortOrder-Util.INC_VALUE;updatedFields.push(sourceField);}else{var targetPreviousIndex=targetIndex-1;var targetPreviousField=array[targetPreviousIndex];if(targetPreviousField.__systemcolumn__){sourceField[sortField]=targetFieldSortOrder-Util.INC_VALUE;updatedFields.push(sourceField);}else{var targetPreviousFieldSortOrder=targetPreviousField[sortField];targetPreviousFieldSortOrder=targetPreviousFieldSortOrder;if(targetFieldSortOrder>targetPreviousFieldSortOrder){var newValue=(targetFieldSortOrder+targetPreviousFieldSortOrder)/2;sourceField[sortField]=newValue;updatedFields.push(sourceField);}else if(targetFieldSortOrder<targetPreviousFieldSortOrder){throw new Error("Target previous index can never be greater than target index")}else{var counter=2;var lowerValue=undefined;var lastIndex=undefined;for(var i=targetIndex-1;i>=0;i--){var iteratorField=array[i];if(iteratorField.__systemcolumn__){break;}
var iteratorFieldSortOrder=iteratorField[sortField];iteratorFieldSortOrder=iteratorFieldSortOrder;if(iteratorFieldSortOrder<targetFieldSortOrder){lowerValue=iteratorFieldSortOrder;lastIndex=i+1;break;}else if(iteratorFieldSortOrder>targetFieldSortOrder){throw new Error("Target previous index can never be greater than target index")}else{counter+=1;lastIndex=i;}}
if(lowerValue==undefined){lowerValue=targetFieldSortOrder-Util.INC_VALUE;}
var difference=(targetFieldSortOrder-lowerValue)/counter;var cumulativeDiff=targetFieldSortOrder-difference;sourceField[sortField]=cumulativeDiff;updatedFields.push(sourceField);for(var i=targetIndex-1;i>=lastIndex;i--){cumulativeDiff-=difference;array[i][sortField]=cumulativeDiff;updatedFields.push(array[i]);}}}}
return updatedFields;}
Util.changeIndex=function(array,indexField,srcIndex,targetIndex){if(!array||array.length<2){return;}
if(srcIndex<0||srcIndex>=array.length||targetIndex<0||targetIndex>=array.length){throw new Error("Index out of bound ");}
if(srcIndex===targetIndex){return;}
var direction="";if(srcIndex<targetIndex){var result=handleDown(array,indexField,srcIndex,targetIndex);return result;}else{var result=handleUp(array,indexField,srcIndex,targetIndex);return result;}}
Util.fkQuery=function(db,query,field,value,queryOptions){function getFkData(query,options){if(queryOptions&&queryOptions.token){options=options||{};options.token=queryOptions.token;}
if(queryOptions&&queryOptions.viewId){options=options||{};options.viewId=queryOptions.viewId;}
return db.query(query,options);}
function verifyFkResult(query,key,newData){var options={};if(queryOptions&&queryOptions.token){options.token=queryOptions.token;}
if(queryOptions&&queryOptions.viewId){options.viewId=queryOptions.viewId;}
options.cache=false;db.query(query,options).then(function(queryResult){var queryRes=queryResult&&queryResult.response&&queryResult.response.result?queryResult.response.result:[];if(JSON.stringify(newData.response.result)!==JSON.stringify(queryRes)){db.removeCache(key);}});}
function filterFkData(result,field,value){if(result&&result.response&&result.response.result.length>0){var data=result.response.result;var length=data.length;var newResult=[];if(value){var lowerCaseValue=value.toLowerCase();for(var i=0;i<length;i++){if(data[i][field]&&data[i][field].toLowerCase().indexOf(lowerCaseValue)==0){newResult.push(data[i]);}}}else{newResult=data;}
return{response:{result:newResult}};}}
function prepareFkOptions(query,value,field,queryOptions,options){var Q=require("q");var d=Q.defer();if(value&&value.trim().length>0){query.$filter=query.$filter||{};value=formatValue(value);var filterValue={"$regex":"^"+value+"","$options":"i"};if(queryOptions.otherDisplayFields&&queryOptions.otherDisplayFields.length>0){var orFilterArray=[];var filter={};filter[field]=filterValue;orFilterArray.push(filter);for(var i=0;i<queryOptions.otherDisplayFields.length;i++){var otherDisplayField=queryOptions.otherDisplayFields[i];var otherDisplayFilter={};otherDisplayFilter[otherDisplayField]=filterValue;orFilterArray.push(otherDisplayFilter);}
delete queryOptions.otherDisplayFields;if(query.$filter.$or){query.$filter.$and=query.$filter.$and||[];query.$filter.$and.push({$or:orFilterArray});}else{query.$filter.$or=orFilterArray;}}else{query.$filter[field]=filterValue;}
options.cache=false;}else{options.cache=true;}
query.$filter&&Object.keys(query.$filter).length>0?options.cache=false:options.cache=true;if(queryOptions&&(!queryOptions.cacheEnabled)){options.cache=false;}
d.resolve();return d.promise;}
function formatValue(value){if(value.indexOf('(')>=0){value=value.replace('(','\\(');}
if(value.indexOf(')')>=0){value=value.replace(')','\\)');}
return value;}
var key="Query-"+JSON.stringify(query);var options={};return prepareFkOptions(query,value,field,queryOptions,options).then(function(){var result=db.getCache(key);return filterFkData(result,field,value);}).then(function(newData){if(newData&&newData.response.result.length>0){verifyFkResult(query,key,newData);return newData;}else{return getFkData(query,options);}});}
Util.asyncIterator=function(array,task){var D=Q.defer();if(!array||array.length==0){D.resolve();return D.promise;}
try{var p=[];var resolvedPromises=0;var errors=[];var onResolve=function(){finalResolve();};var onReject=function(err){var errDetail={};errDetail.message=err.message;errDetail.stack=err.stack;errors.push(errDetail);finalResolve();};var finalResolve=function(){resolvedPromises=resolvedPromises+1;if(resolvedPromises===array.length){if(errors.length>0){D.reject(new Error(errors.length>1?JSON.stringify(errors):JSON.stringify(errors[0])))}else{D.resolve();}}};for(var i=0;i<array.length;i++){callTask(i,array[i],task).then(onResolve).fail(onReject);}}catch(e){D.reject(e);}
return D.promise;};function callTask(i,record,task){var D=Q.defer();setTimeout(function(){var retValue=task(i,record);if(Q.isPromise(retValue)){retValue.then(function(val){D.resolve(val);}).fail(function(err){D.reject(err);});}else{D.resolve(retValue);}},10);return D.promise;}
Util.removeSpecialChar=function(inputString){if(inputString){var outputString=inputString.replace(/([~!@#$%^&*()_+=`{}\[\]\|\\:;'<>,.\/? ])+/g,'_').replace(/^(_)+|(_)+$/g,'');return outputString;}}
Util.replaceDollarAndThis=function(data,changeTo){if(data===true||data===false||data===null||data===undefined){return data;}
if(typeof data==="string"&&data.indexOf("$$")<0){data=data.toString();changeTo=changeTo||"row.entity.";data=data.replace(/\$/g,changeTo).replace(/\'/g,"\"");data=data.replace(/this./g,changeTo).replace(/\'/g,"\"");}
return data;};(function(definition){if(typeof exports==="object"){module.exports=definition();}else{ModuleManager=definition();}})(function(){"use strict";function ModuleManager(){}
ModuleManager.triggerModules=function(event,document,collection,db,options){var EventManager=require("ApplaneDB/lib/EventManager.js");return EventManager.triggerEvents(event,document,collection.events,collection,db,options)}
