...
var FILTERABLE_COLUMN = 'filterable';
...
var UI_TYPE_GEO_LOCATION = 'geolocation';
...
AppUtil.populateColumn = function (column, viewType, model, filters, appliedfilters, $scope, $dataModel) {
    ...
    if (type == UI_TYPE_TABLE) {
        ...
        var atomic = false;
        /*for lookup if atomic is true, then we will not remove the previous value on selection in lookup directive, required in crm to change contact detail name*/
        var bindtype = false;
        if (type == UI_TYPE_STRING && column.lookupquery) {
            ...
            if (column.lookupquery.field) {
                field = column.lookupquery.field;
                atomic = column.lookupquery.atomic;
            } else {
        ...
        if (column[FILTERABLE_COLUMN] && filters && appliedfilters) {
...
AppUtil.handleToolBarHeight = function (componentid) {
    setTimeout(function () {
        var rightToolBarWidth = $('#right_tool_bar_' + componentid).width();
        var leftToolBarWidth = $('#left_tool_bar_' + componentid).width();
        var remainingWidth = Math.abs($(window).width() - (rightToolBarWidth + leftToolBarWidth));
        var userPreferencePlateWidth = $('#user_prefrence_plate_' + componentid).width();
        if (userPreferencePlateWidth > remainingWidth) {
            $('#user_prefrence_plate_parent' + componentid).css({left:0, right:0, top:"30px"});
            $(VIEW_CONTAINER).css({top:(121 + $('#user_prefrence_plate_parent' + componentid).height()) + "px"});   // default top of container div
        } else {
            $('#user_prefrence_plate_parent' + componentid).attr("style", "top:0px;");
...
AppUtil.getDataFromService = function (url, requestBody, callType, dataType, pBusyMessage, callback, errcallback) {
    ...
    var state = {timezone:new Date().getTimezoneOffset()};
    requestBody.state = JSON.stringify(state);
                ...
                pre:function ($scope, iElement, attrs) {
                    ...
                    $scope.getColumnValue = function (row, col, asHTML) {
                        ...
                        if (row && exp && colValue !== undefined && colValue !== null) {
                            ...
                            if (type == UI_TYPE_LOOK_UP) {
                                ...
                                val = colValue;
                            } else if (type == UI_TYPE_GEO_LOCATION) {
                                val = colValue;
                            } else {
                                val = colValue
                            }
                    ...
                    if ($scope.view.metadata.actions && $scope.view.metadata.actions.length > 0) {
                        ...
                        for (var i = 0; i < $scope.view.metadata.actions.length; i++) {
                            ...
                            if (action.type == "Filter" && action.filterType) {
                                action[UI] = action.filterType;
                                action[FILTERABLE_COLUMN] = true;
                    ...
                    $scope.userActions = function ($event) {
                        ...
                        var columns = [
                            ...
                            {label:"Filter Type", expression:"filterType", ui:UI_TYPE_LOOK_UP, width:150, options:["lookup", "date"], showOnPanel:true, visibleExpressionPanel:"this.type == 'Filter'"} ,
...
//                        viewInfo[PARENT_COMPONENT_ID] = $scope.componentid;
                    ...
                    $scope.columnview = function ($event) {
                        ...
                        var uiOptions = ['table', 'duration', 'compositelookup', 'currency', 'date', 'number', 'decimal', 'string', 'lookup', 'image', 'file', 'selection', 'sr', 'index', 'boolean', 'range', 'aggregate', 'time', 'text', 'autoheight', 'richtext', 'json', 'schedule'];
                        ...
                        if ($scope.appData.applicationDeveloper) {
                            columns.push({expression:"ftsEnable", ui:UI_TYPE_BOOLEAN, label:"FTS Enable", width:100, showOnPanel:true});
                            ...
                            columns.push({expression:"filterable", ui:UI_TYPE_BOOLEAN, label:"Filterable", width:100, showOnPanel:true});
                            ...
                            columns.push({options:['sum'], label:"Aggregate", expression:"totalaggregates", ui:UI_TYPE_STRING, width:50, showOnPanel:true, visibleExpressionPanel:"this.ui == 'number' || this.ui == 'decimal' || this.ui == 'duration' || this.ui == 'currency'"});
                        ...
                        $timeout(function () {
                            ...
                            if (!asDefault) {
                                ...
                                Object.keys(newFilterParams).forEach(function (k) {
                                    if (newFilterParams[k] && !newFilterParams[k].asParameter) {
                                        $scope.view.metadata.parameters[k] = newFilterParams[k];
                                    }
                                });


                                AppUtil.handleToolBarHeight($scope[COMPONENT_ID]);
                                var url = BAAS_SERVER + "/custom/module";
                                var parameters = {"ask":$scope.view.ask, "osk":$scope.view.osk, "viewid":$scope.view.viewid};
...
//                        }
                    ...
                    $scope.print = function () {
                        var html = $scope.view.data.__html;
                        var w = window.open();
...
//                        $dataModel.deleteData($scope.componentid);
                    ...
                    $scope.update = function () {
                        ...
                        for (var i = 0; i < columnCount; i++) {
                            ...
                            if (col.update) {
                                var colClone = angular.copy(col);
                                colClone[SHOW_ON_PANEL] = true;
                                panelColumns.push(colClone);
                            }
                        }
        ...
        compile:function () {
            return {
                pre:function ($scope, iElement) {
                    ...
                    $scope.toggleTree = function (row, index) {
                        $dataModel.toggleTree(row, $scope.componentid, index);
                    };
...
//                                                    $scope.view.metadata.columns.splice($scope.view.metadata.columns.length - 1, 0, column);
                    ...
                    $scope.populateMetaData = function () {
                        ...
                        var rowactions = [];
                        var groupColumns = [];
                        var showDetail = AppUtil.isTrueOrUndefined(metadata.enablerowaction);

                        if (showDetail) {
                            showDetail = false;
                            for (var i = 0; i < metadata.columns.length; i++) {
                                if (metadata.columns[i][SHOW_ON_PANEL]) {
                                    showDetail = true;
                                    break;
                                }
                                ;
                        ...
                        for (var i = 0; i < columnCount; i++) {
                            ...
                            ftsEnable = ftsEnable || column.ftsEnable;
...
//                                    }
                                    AppUtil.populateColumn(metadata.userFilters[i], $scope.view.metadata.type, "row", filters, appliedfilters, $scope, $dataModel);
                    ...
                    try {
                        ...
                        if (headerscroll) {
                            ...
                            template += "<table style='width: 100%;' cellpadding='0' cellspacing='0'>" +
                                ...
                                "<td applane-grid-cell class='applane-grid-cell draggable' ng-class='isValidColumn(row,col)' ng-repeat=' col in view.metadata.tableColumns' tabindex=0 ng-style='col.style' style='max-width:100px;'>" +
                                "</td>" +
                                "</tr>";
                            for (var i = 0; i < $scope.view.metadata.tableColumns.length; i++) {
                                if ($scope.view.metadata.tableColumns[i][TOTAL_AGGREGATES]) {
                                    template += "<tr ng-show='" + aggregateExpression + "' class='app-border-top-black app-border-bottom-black' ng-init='row=" + aggregateExpression + "'>" +
                                        "<td style='border:none;' class='applane-grid-cell draggable' ng-repeat='col in view.metadata.tableColumns' ng-bind-html-unsafe='getColumnValue(row,col,false)' ng-style='col.style'>" +
                                        "</tr>";
                                    break;
                                }
                            }
                            ...
                            if (filters && filters.length > 0)
                                for (var i = 0; i < filters.length; i++) {
                                    ...
                                    if (!ui && filters[i].type) {
                                        ...
                                        nestedMetaData.defaultFilters.filters[i][FILTERABLE_COLUMN] = true;
                                    }
                                }
                        }
...
appStrapServices.factory('$viewStack', [
    '$rootScope', '$compile', '$http', '$timeout', '$dataModel', function ($rootScope, $compile, $http, $timeout, $dataModel) {
        ...
        $viewStack.addView = function (view, $scope) {
            ...
            if (view.metadata) {
                if (view.metadata.type == "html") {
                    view.metadata.resized = true;
                }
...
//                });
        ...
        $viewStack.closeView = function (componentId) {
            ...
            if (showInPopup || scope.view.metadata.embedPanel) {
                ...
                AppUtil.handleToolBarHeight(view[PARENT_COMPONENT_ID]);
            }
...
appStrapServices.factory('$dataModel', [
    '$http', '$timeout', '$rootScope', '$appService', '$cacheDataModel', function ($http, $timeout, $rootScope, $appService, $cacheDataModel) {
        ...
        $dataModel.isFilterApplied = function (expression, filters) {
            ...
            for (var k in filters) {
                if (k == expression || k.indexOf(expression + ".") == 0) {
                    ...
                    if (AppUtil.resolve(filters, expression) !== undefined) {
                        return true;
                    }
                }
            }
        ...
        $dataModel.toggleTree = function (row, id, index) {
            var model = this[id];
            var data = AppUtil.resolve(model.scope, model.scope.view.metadata.dataExpression);
...
//dotted exp will add in nested expression view and simple will be added in detail view
        ...
        $dataModel.handleAggregateChanges = function (options) {
            ...
            if (type != "currency" && type != "duration" && type != "number" && type != "decimal") {
                throw new Error("Unhandled typed in aggregate[" + type + "]");
            }
            var valAsObject = true;
...
//            }
            ...
            var aggregateVal = undefined;
            var aggregateType = undefined;
            if (valAsObject) {
                oldVal = AppUtil.resolve(oldColumnValue, valExp);
                oldType = AppUtil.resolve(oldColumnValue, typeExp);
                newVal = AppUtil.resolve(newColumnvalue, valExp);
                newType = AppUtil.resolve(newColumnvalue, typeExp);
                aggregateVal = AppUtil.resolve(aggregateColumnValue, valExp);
                aggregateType = AppUtil.resolve(aggregateColumnValue, typeExp);
            } else {
                oldVal = oldColumnValue;
                newVal = newColumnvalue;
                aggregateVal = aggregateColumnValue;
            }
...
//if row got deleted then we will process only aggregate changes
        ...
        $dataModel.processValidations = function (componentId, currentRow, $scope) {
            ...
            for (var i = 0; i < model.view.metadata.validations.length; i++) {
                ...
                if (!column[SHOW_ON_TABLE] || !column[SHOW_ON_PANEL]) {
                    continue;
                }

                if (column[EDITABLE_EXPRESSION] !== undefined || column[EDITABLE_EXPRESSION_PANEL] !== undefined) {
                    continue;
                }

                if (column[VISIBLE_EXPRESSION] !== undefined || column[VISIBLE_EXPRESSION_PANEL] !== undefined) {
                    continue;
                }
        ...
        $dataModel.refresh = function (componentid, refreshCallback) {
            ...
            if ((viewType == 'panel' && view.metadata.columns)) {
                for (var i = 0; i < view.metadata.columns.length; i++) {
                    if (view.metadata.max_rows && view.metadata.max_rows == 1 && view.metadata.columns[i][SHOW_ON_PANEL]) {
                        refreshRequired = true;
                        columns[view.metadata.columns[i].expression] = 1;
                    } else if ((view.metadata.columns[i][SHOW_ON_PANEL] && !view.metadata.columns[i][SHOW_ON_TABLE])) {
                        refreshRequired = true;
                        columns[view.metadata.columns[i].expression] = 1;
                    }
                }
...
//                }
        ...
        $dataModel.modifyDataAsPerColumn = function (data, columns) {
            ...
            var geoColumnIndex = AppUtil.getIndexFromArray(columns, "ui", UI_TYPE_GEO_LOCATION);
            ...
            } else if ((jsonColumnIndex !== undefined && jsonColumnIndex >= 0) || (geoColumnIndex !== undefined && geoColumnIndex >= 0)) {
                iterateRequired = true;
            }
            ...
            for (var i = 0; i < data.length; i++) {
                if (compositeColumn) {
                    ...
                    for (var j = 0; j < columns.length; j++) {
                        var obj = columns[j];
                        if (obj.ui == UI_TYPE_JSON || obj.ui == UI_TYPE_GEO_LOCATION) {
                            $dataModel.resolveJSON(data[i], obj.expression);
                        }
        ...
        $dataModel.resolveJSON = function (data, expression) {
            if (!expression || !data) {
                return;
            }
            var dotIndex = expression.indexOf(".");
            if (dotIndex >= 0) {
                var firstExpression = expression.substring(0, dotIndex);
                var nextExp = expression.substring(dotIndex + 1);
                var firstExpressionValue = AppUtil.resolve(data, firstExpression);
                if (firstExpressionValue) {
                    if (angular.isArray(firstExpressionValue)) {
                        for (var i = 0; i < firstExpressionValue.length; i++) {
                            $dataModel.resolveJSON(firstExpressionValue[i], nextExp)

                        }
                    } else if (angular.isObject(firstExpressionValue)) {
                        $dataModel.resolveJSON(firstExpressionValue, nextExp)
                    }
                }
            } else {
                var jsonColumnColumnValue = AppUtil.resolve(data, expression);
                if (jsonColumnColumnValue && (angular.isArray(jsonColumnColumnValue) || angular.isObject(jsonColumnColumnValue))) {
                    AppUtil.putDottedValue(data, expression, JSON.stringify(jsonColumnColumnValue));
                }
            }
        }
...
//            }
            ...
            if (!angular.equals(oldValue, newValue)) {
                ...
                } else if (type == UI_TYPE_JSON || type == UI_TYPE_GEO_LOCATION) {
        ...
        $dataModel.updateColumnClone = function (componentid, columns, metadata) {
            ...
            for (var i = 0; i < columnsLength; i++) {
                ...
                if (column.filterRequiredColumns) {
                    $dataModel.pushDependentColumn(column.filterRequiredColumns, columns);
                }
            ...
            compile:function () {
                return {
                    post:function ($scope, iElement, iAttr) {
                        ...
                        template += '<tr ';
                        if ($scope.view.metadata.$recursivecolumn || $scope.view.metadata.groupColumns) {
                            template += " ng-show='!row._hidden'";
                        }
                        if ($scope.view.metadata.groupColumns && $scope.view.metadata.groupColumns.length > 0) {
                            template += " ng-class=\"{'app-row{{row._level}}' : row._mode > 0}\" ";
                        }
                        template += " style='height:25px;' class='draggable' ng-class-even=\"'applane-grid-row-even'\" ng-class-odd=\"'applane-grid-row-odd'\" ng-repeat='row in " + dataExpression + "'>" +
                            "<td applane-grid-cell class='applane-grid-cell' ng-class='isValidColumn(row,col)' ng-repeat='col in view.metadata.tableColumns' tabindex=0 ng-style='col.style' >" +
                            "</td>" +
                            "</tr>";
                        for (var i = 0; i < $scope.view.metadata.tableColumns.length; i++) {
                            if ($scope.view.metadata.tableColumns[i][TOTAL_AGGREGATES]) {
                                template += "<tr ng-show='" + aggregateExpression + "' class='app-border-top-black app-border-bottom-black' ng-init='row=" + aggregateExpression + "'>" +
                                    "<td style='border:none;' class='applane-grid-cell' ng-repeat='col in view.metadata.tableColumns' ng-style='col.style' ng-bind-html-unsafe='getColumnValue(row,col,false)'>" +
                                    "</tr>";
                                break;
                            }
                        }
                        template += "</tbody></table>" + "<div class='app-padding-five-px app-width-fifty-px app-position-fixed app-border app-light-gray-backgroud-color app-font-weight-bold app-box-shadow app-text-align-center app-display-none' id='{{componentid}}rowDrag'></div>";
                        $(iElement).append($compile(template)($scope));
                    }
                }
            }
            ...
            compile:function () {
                return {
                    pre:function ($scope, iElement, iAttr) {
                        ...
                        if ($scope.col.options && $scope.col.options.length > 0) {
                            $(iElement).hover(function () {
                                var th = $("#" + $scope[COMPONENT_ID] + "_head").find("th");
                                $(th[iElement.index()]).find('.app-column-options').attr('style', 'visibility:visible;');
                            }, function () {
                                var th = $("#" + $scope[COMPONENT_ID] + "_head").find("th");
                                $(th[iElement.index()]).find('.app-column-options').removeAttr("style");
                            });
                        }
                        ...
                        $scope.showRenderingCell = function () {
                            ...
                            if ($scope.view.metadata.groupColumns && $scope.view.metadata.groupColumns.length > 0 && $scope.row._group !== undefined) {
                                ...
                                if ($scope.col.$groupcolumn) {
                                    ...
                                    var treeNodeTemplate = "<span ng-style='{\"padding-left\":row._level * 20+\"px\"}' class='app-float-left app-text-align-center app-cursor-pointer' ng-click='toggleTree(row, $parent.$parent.$index)' ng-class=\"{'app-minus':row._mode == 1, 'app-plus':row._mode == 0}\" ng-show='row._mode == 0 || row._mode == 1'>&nbsp;</span>";
                                ...
                                if ($scope.row._mode == 0 || $scope.row._mode == 1) {
                                    var treeNodeTemplate = "<span ng-style='{\"padding-left\":row._level * 20+\"px\"}' class='app-float-left app-text-align-center app-cursor-pointer' ng-click='toggleTree(row, $parent.$parent.$index)' ng-class=\"{'app-minus':row._mode == 1, 'app-plus':row._mode == 0}\" >&nbsp;</span>";
                                ...
                                cellTemplate = "<div ng-dblclick='editCell($event)' title='{{getColumnValue(row,col,false)}}' class='applane-grid-cell-inner app-overflow-hiiden app-white-space-nowrap' ";
            ...
            compile:function () {
                return {
                    pre:function ($scope, iElement, iAttr) {
                        ...
                        if ($scope.col.options && $scope.col.options.length > 0) {
                            cellTemplate += "<div class='app-column-options' ng-click='columnOptions($event)'></div>";
                        }
                        cellTemplate += '</div>';
                        if ($scope.col[UI] == UI_TYPE_SELECTION) {
                        ...
                        $scope.columnOptions = function ($event) {
                            if (!$scope.col.options || $scope.col.options.length == 0) {
                                return;
                            }

                            var optionsHtml = "<div style='width:100px;'>";

                            for (var i = 0; i < $scope.col.options.length; i++) {
                                optionsHtml += "<div>" + $scope.col.options[i] + "</div>";
                            }
                            optionsHtml += "</div>";
                            ...
                            var p = new Popup({
                                ...
                                escEnabled:true,
                                hideOnClick:true,
                                html:$compile(optionsHtml)($scope.$new()),
                                scope:$scope.$new(),
                                element:$event.target
                            });
                            p.showPopup();
        ...
        compile:function () {
            return {
                post:function ($scope, iElement) {
                    ...
                    html += '<div id="right_tool_bar_{{componentid}}" class="app-float-right" style="height:30px;">';
                    ...
                    html += '</div>';
                    html += '<div id="left_tool_bar_{{componentid}}" class="app-float-left">';
            ...
            compile:function () {
                return {
                    pre:function ($scope, iElement) {
                        ...
                        if (AppUtil.isTrueOrUndefined($scope.view.metadata.type == 'html')) {
                            actions.push({"method":"print()", "label":"Print", "template":"<div class='app-print-wrapper'>" + DEFAULT_ACTION_TEMPLATE + "</div>", "class":"app-print-button app-bar-button"});
                        }

                        if ($scope.view.metadata.ftsSearch) {
                            actions.push({"method":"", "label":"", "template":"<app-fts-search></app-fts-search>", "class":""});
                        }
                        ...
                        actions.push({"method":"", "label":"", "template":"<app-user-prefrence-bar></app-user-prefrence-bar>", "class":""});
                        var actionCount = actions.length;
                        for (var i = 0; i < actionCount; i++) {
                            var action = actions[i];
                            var actionLabel = action.label;
...
appStrapDirectives.directive('appUserPrefrenceBar', [
    '$compile', function ($compile) {
        return {
            restrict:'E',
            replace:true,
            ...
            compile:function () {
                return {
                    pre:function ($scope, iElement) {
                        ...
                        var html = "<div class='app-float-left'>" +
                            "<div class='app-float-left app-position-absolute app-user-preference' id='user_prefrence_plate_parent{{componentid}}'>" +
                            "<div id='user_prefrence_plate_{{componentid}}' class='app-float-left'>";

                        if ($scope.view.metadata.userHeaderActions && $scope.view.metadata.userHeaderActions.length > 0) {
                            html += "<app-user-actions></app-user-actions>";
                        }
            ...
            return {
                post:function ($scope, iElement) {
                    ...
                    var template = "<div ng-repeat='action in view.metadata.userHeaderActions' class='app-float-left'>" +
                        "<div ng-click='userActions($index)' class='app-float-left app-cursor-pointer app-populate-default-filter app-font-weight-bold app-blue-text-color app-invoke-method' ng-bind='action.label' ng-show = 'action.visibleExpression'    ></div>" +
                        "</div>";
                    iElement.append(($compile)(template)($scope));
                },
                pre:function ($scope) {
                    if ($scope.view.metadata.userHeaderActions && $scope.view.metadata.userHeaderActions.length > 0) {
                        for (var i = 0; i < $scope.view.metadata.userHeaderActions.length; i++) {
                            ...
                            action.visibleExpression = 'true';
                            if (action.jobname) {
                                var jobs = ["updateOrderStatus", "updateInvoiceStatus", "updateShipmentStatus", "updateReceiptStatus"];
                                if (jobs.indexOf(action.jobname) >= 0) {
                                    if ($scope.appData.username == 'varun@torerocorp.com' || $scope.appData.username.toLowerCase() == 'kalyan@torerocorp.com') {
                                        action.visibleExpression = true;
                                    } else {
                                        action.visibleExpression = false;
                                    }
                                }
