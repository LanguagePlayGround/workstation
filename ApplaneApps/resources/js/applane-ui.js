            ...
            return {
                ...
                post:function ($scope, iElement) {
                    ...
                    for (var i = 0; i < metadata.columns.length; i++) {
                        ...
                        if (!column[SHOW_ON_PANEL] || column.pexpression) {
                            continue;
                        }
                        if ($scope.view.metadata.embedPanel && column[UI_PANEL] == UI_TYPE_TABLE) {
                            continue;
                        }
                    ...
                    for (var i = 0; i < panelColumns.length; i++) {
                        ...
                        if (!($dataModel.isFilterApplied(filterExp, $scope.view.metadata.filterparameters)) && $dataModel.isFilterApplied(filterExp, $scope.view.metadata.filter)) {
                            if (column.ui !== UI_TYPE_DATE && column.ui !== UI_TYPE_SCHEDULE) {
                                continue;
                            }
                    ...
                    $timeout(function () {
                        ...
                        if ($scope.view.metadata.max_rows && $scope.view.metadata.max_rows == 1) {
                            if ($scope.view.metadata.resized === undefined || $scope.view.metadata.resized) {
                                $scope.view.metadata.resized = true;
                                $('#' + $scope.view[COMPONENT_ID]).css({left:0});
                            } else {
                                $scope.view.metadata.resized = false;
                            }
                        }
...
//                            columnWidth = "100%";
                        ...
                        template += " ></app-column>";
                        if (editableExpression !== undefined && editableExpression.toString().length > 0) {
                            template += '<div style="min-width:200px;min-height:25px;padding-top:5px;" ng-bind="getColumnValue(row,col,false);" ng-show="!(' + editableExpression + ')" ng-class=\'{"app-height-nineteen": view.showas == "popup"}\'></div>';
                        }
            ...
            return  {
                post:function ($scope, iElement, attrs) {
                    ...
                    $scope.nLastDaysFilter = function () {
                        $scope.hideDateFilterPopUp();
                        var days = AppUtil.resolve($scope, toBind + ".nLastDays", true, false);
                    ...
                    $scope.rengeFilter = function () {
                        $scope.hideDateFilterPopUp();
                        var fromLabel = AppUtil.resolve($scope, toBind + ".fromlabel", true, false);
                        var toLabel = AppUtil.resolve($scope, toBind + ".tolabel", true, false);
...
//ignore exception in matching
                    ...
                    $scope.updater = function (value) {
                        ...
                        if (multiple) {
                            ...
                            $scope.$element.val(v);
                            if (bindType == 'object') {
                                if (attrs.atomic === undefined || attrs.atomic !== 'true') {
...
//                                    })
                                    ...
                                    if (value === undefined || value === null || value.trim().length == 0) {
                                        ...
                                        if (attrs.atomic === undefined || attrs.atomic !== 'true') {
                                            ...
                                            AppUtil.putDottedValue(model, fieldExpression + "." + displayExpression, value);
                                        }
...
appStrapDirectives.directive('appBreadCrumb', ["$compile", "$viewStack", function ($compile, $viewStack) {
    return {
        ...
        replace:true,
        scope:true,
        template:"<div class='app-float-left app-breadcrumb'>" +
            "<div class='app-float-left' ng-repeat='breadCrumb in view.metadata.breadCrumbInfo'>" +
            "<div class='app-float-left app-font-weight-bold app-color-blue' style='font-size:19px;line-height:27px;' ng-show='$index>0'> &#8674; </div>" +
            ...
            return {
                post:function ($scope, iElement) {
                    ...
                    for (var i = 0; i < $scope.view.metadata.columns.length; i++) {
                        ...
                        if (column && column.label && column.ftsEnable) {
                            $scope.ftsDataSource.push(column.label + ":");
                        }
                    }

                    var template = "<div class='app-float-left app-border fts-search'>" +
